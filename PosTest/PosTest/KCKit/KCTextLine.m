...
- (void)reloadBounds {
    ...
    for (NSUInteger r = 0; r < runCount; r++) {
        ...
        KCTextAttachment *attachment = attrs[KCTextAttachmentAttributeName];
        if (attachment) {
            ...
            NSRange runRange = KCNSRangeFromCFRange(CTRunGetStringRange(run));
...
- (NSString *)description {
    ...
    [desc appendFormat:@"<KCTextLine: %p> row:%zd range:%tu,%tu",self, self.row, range.location, range.length];
