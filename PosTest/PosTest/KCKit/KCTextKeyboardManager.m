...
static int _KCTextKeyboardViewFrameObserverKey;

@interface _KCTextKeyboardViewFrameObserver : NSObject
...
@implementation _KCTextKeyboardViewFrameObserver{
...
- (void)addToKeyboardView:(UIView *)keyboardView{
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_KCTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_KCTextKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_KCTextKeyboardViewFrameObserverKey);
}
...
@implementation KCTextKeyboardManager{
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"TextKeyboardManager初始化报错" reason:@"实例调用defaultManager" userInfo:nil];
    return [super init];
}
- (instancetype)_init {
    self = [super init];
...
- (void)_initFrameObserver {
    ...
    _KCTextKeyboardViewFrameObserver *observer = [_KCTextKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_KCTextKeyboardViewFrameObserver new];
...
+ (instancetype)defaultManager{
    ...
    static KCTextKeyboardManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (![UIApplication isAppExtension]) manager = [[self alloc]_init];
    });
    return manager;
...
- (void)_notifyAllObservers {
    ...
    KCTextKeyboardTransition trans = {0};
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<KCTextKeyboardObserver> observer in _observers) {
