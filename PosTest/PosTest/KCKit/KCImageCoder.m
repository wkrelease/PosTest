...
#define KC_FOUR_CC(c1,c2,c3,c4) ((uint32_t)(((c4) << 24) | ((c3) << 16) | ((c2) << 8) | (c1)))
#define KC_TWO_CC(c1,c2) ((uint16_t)(((c2) << 8) | (c1)))

static inline uint16_t kc_swap_endian_uint16(uint16_t value) {
    return
    (uint16_t) ((value & 0x00FF) << 8) |
    (uint16_t) ((value & 0xFF00) >> 8) ;
}

static inline uint32_t kc_swap_endian_uint32(uint32_t value) {
    return
...
typedef enum {
    KC_PNG_ALPHA_TYPE_PALEETE = 1 << 0,
    KC_PNG_ALPHA_TYPE_COLOR = 1 << 1,
    KC_PNG_ALPHA_TYPE_ALPHA = 1 << 2,
} kc_png_alpha_type;

typedef enum {
    KC_PNG_DISPOSE_OP_NONE = 0,
    KC_PNG_DISPOSE_OP_BACKGROUND = 1,
    KC_PNG_DISPOSE_OP_PREVIOUS = 2,
} kc_png_dispose_op;

typedef enum {
    KC_PNG_BLEND_OP_SOURCE = 0,
    KC_PNG_BLEND_OP_OVER = 1,
} kc_png_blend_op;
...
typedef struct {
    ...
    kc_png_chunk_fcTL frame_control;
} kc_png_frame_info;

typedef struct {
    kc_png_chunk_IHDR header;   ///< png header
    kc_png_chunk_info *chunks;      ///< chunks
    uint32_t chunk_num;          ///< count of chunks
    
    kc_png_frame_info *apng_frames; ///< frame info, NULL if not apng
...
static void kc_png_chunk_IHDR_read(kc_png_chunk_IHDR *IHDR, const uint8_t *data) {
    IHDR->width = kc_swap_endian_uint32(*((uint32_t *)(data)));
    IHDR->height = kc_swap_endian_uint32(*((uint32_t *)(data + 4)));
...
static void kc_png_chunk_IHDR_write(kc_png_chunk_IHDR *IHDR, uint8_t *data) {
    *((uint32_t *)(data)) = kc_swap_endian_uint32(IHDR->width);
    *((uint32_t *)(data + 4)) = kc_swap_endian_uint32(IHDR->height);
...
static void kc_png_chunk_fcTL_read(kc_png_chunk_fcTL *fcTL, const uint8_t *data) {
    fcTL->sequence_number = kc_swap_endian_uint32(*((uint32_t *)(data)));
    fcTL->width = kc_swap_endian_uint32(*((uint32_t *)(data + 4)));
    fcTL->height = kc_swap_endian_uint32(*((uint32_t *)(data + 8)));
    fcTL->x_offset = kc_swap_endian_uint32(*((uint32_t *)(data + 12)));
    fcTL->y_offset = kc_swap_endian_uint32(*((uint32_t *)(data + 16)));
    fcTL->delay_num = kc_swap_endian_uint16(*((uint16_t *)(data + 20)));
    fcTL->delay_den = kc_swap_endian_uint16(*((uint16_t *)(data + 22)));
    fcTL->dispose_op = data[24];
    fcTL->blend_op = data[25];
}

static void kc_png_chunk_fcTL_write(kc_png_chunk_fcTL *fcTL, uint8_t *data) {
    *((uint32_t *)(data)) = kc_swap_endian_uint32(fcTL->sequence_number);
    *((uint32_t *)(data + 4)) = kc_swap_endian_uint32(fcTL->width);
    *((uint32_t *)(data + 8)) = kc_swap_endian_uint32(fcTL->height);
    *((uint32_t *)(data + 12)) = kc_swap_endian_uint32(fcTL->x_offset);
    *((uint32_t *)(data + 16)) = kc_swap_endian_uint32(fcTL->y_offset);
    *((uint16_t *)(data + 20)) = kc_swap_endian_uint16(fcTL->delay_num);
    *((uint16_t *)(data + 22)) = kc_swap_endian_uint16(fcTL->delay_den);
    data[24] = fcTL->dispose_op;
    data[25] = fcTL->blend_op;
}

// convert double value to fraction
static void kc_png_delay_to_fraction(double duration, uint16_t *num, uint16_t *den) {
    if (duration >= 0xFF) {
        *num = 0xFF;
        *den = 1;
...
static double kc_png_delay_to_seconds(uint16_t num, uint16_t den) {
    if (den == 0) {
        return num / 100.0;
    } else {
...
static bool kc_png_validate_animation_chunk_order(kc_png_chunk_info *chunks,  /* input */
    ...
    if (chunks->fourcc != KC_FOUR_CC('I', 'H', 'D', 'R')) return false;
    if ((chunks + chunk_num - 1)->fourcc != KC_FOUR_CC('I', 'E', 'N', 'D')) return false;
    ...
    for (uint32_t i = 0; i < chunk_num; i++) {
        kc_png_chunk_info *chunk = chunks + i;
        switch (chunk->fourcc) {
            case KC_FOUR_CC('I', 'H', 'D', 'R'): {  // png header
                if (i != 0) return false;
                if (IHDR_num > 0) return false;
                IHDR_num++;
            } break;
            case KC_FOUR_CC('I', 'D', 'A', 'T'): {  // png data
                if (prev_fourcc != KC_FOUR_CC('I', 'D', 'A', 'T')) {
                    if (IDAT_num == 0)
                        first_IDAT = i;
                    else
                        return false;
            ...
            case KC_FOUR_CC('a', 'c', 'T', 'L'): {  // apng control
                if (acTL_num > 0) return false;
                acTL_num++;
            } break;
            case KC_FOUR_CC('f', 'c', 'T', 'L'): {  // apng frame control
                if (i + 1 == chunk_num) return false;
                if ((chunk + 1)->fourcc != KC_FOUR_CC('f', 'd', 'A', 'T') &&
                    (chunk + 1)->fourcc != KC_FOUR_CC('I', 'D', 'A', 'T')) {
                    return false;
                }
                if (fcTL_num == 0) {
                    if ((chunk + 1)->fourcc == KC_FOUR_CC('I', 'D', 'A', 'T')) {
                        first_frame_cover = true;
                    }
                }
                fcTL_num++;
            } break;
            case KC_FOUR_CC('f', 'd', 'A', 'T'): {  // apng data
                if (prev_fourcc != KC_FOUR_CC('f', 'd', 'A', 'T') && prev_fourcc != KC_FOUR_CC('f', 'c', 'T', 'L')) {
                    return false;
                }
            } break;
        }
...
static void kc_png_info_release(kc_png_info *info) {
    if (info) {
...
/**
 ...
 @return A png info object, you may call kc_png_info_release() to release it.
 Returns NULL if an error occurs.
 */
static kc_png_info *kc_png_info_create(const uint8_t *data, uint32_t length) {
    if (length < 32) return NULL;
    if (*((uint32_t *)data) != KC_FOUR_CC(0x89, 0x50, 0x4E, 0x47)) return NULL;
    if (*((uint32_t *)(data + 4)) != KC_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A)) return NULL;
    
    uint32_t chunk_realloc_num = 16;
    kc_png_chunk_info *chunks = malloc(sizeof(kc_png_chunk_info) * chunk_realloc_num);
    ...
    do {
        if (chunk_num >= chunk_capacity) {
            kc_png_chunk_info *new_chunks = realloc(chunks, sizeof(kc_png_chunk_info) * (chunk_capacity + chunk_realloc_num));
            if (!new_chunks) {
                free(chunks);
                return NULL;
            }
        ...
        kc_png_chunk_info *chunk = chunks + chunk_num;
        const uint8_t *chunk_data = data + offset;
        chunk->offset = offset;
        chunk->length = kc_swap_endian_uint32(*((uint32_t *)chunk_data));
        ...
        chunk->crc32 = kc_swap_endian_uint32(*((uint32_t *)(chunk_data + 8 + chunk->length)));
        chunk_num++;
        offset += 12 + chunk->length;
        
        switch (chunk->fourcc) {
            case KC_FOUR_CC('a', 'c', 'T', 'L') : {
                if (chunk->length == 8) {
                    apng_frame_number = kc_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
                    apng_loop_num = kc_swap_endian_uint32(*((uint32_t *)(chunk_data + 12)));
                } else {
                    apng_chunk_error = true;
                }
            } break;
            case KC_FOUR_CC('f', 'c', 'T', 'L') :
            case KC_FOUR_CC('f', 'd', 'A', 'T') : {
                if (chunk->fourcc == KC_FOUR_CC('f', 'c', 'T', 'L')) {
                    if (chunk->length != 26) {
                        apng_chunk_error = true;
                    } else {
                        apng_frame_index++;
                ...
                if (chunk->length > 4) {
                    uint32_t sequence = kc_swap_endian_uint32(*((uint32_t *)(chunk_data + 8)));
    ...
    if (chunk_num < 3 ||
        chunks->fourcc != KC_FOUR_CC('I', 'H', 'D', 'R') ||
        chunks->length != 13) {
        free(chunks);
        return NULL;
    }
    ...
    // png info
    kc_png_info *info = calloc(1, sizeof(kc_png_info));
    if (!info) {
        free(chunks);
        return NULL;
    }
    ...
    kc_png_chunk_IHDR_read(&info->header, data + chunks->offset + 8);
    ...
    if (!apng_chunk_error && apng_frame_number == apng_frame_index && apng_frame_number >= 1) {
        ...
        if (!kc_png_validate_animation_chunk_order(info->chunks, info->chunk_num, &first_IDAT_index, &first_frame_is_cover)) {
        ...
        info->apng_frames = calloc(apng_frame_number, sizeof(kc_png_frame_info));
        if (!info->apng_frames) {
            kc_png_info_release(info);
            return NULL;
        }
        ...
        for (int32_t i = 0; i < info->chunk_num; i++) {
            kc_png_chunk_info *chunk = info->chunks + i;
            switch (chunk->fourcc) {
                case KC_FOUR_CC('I', 'D', 'A', 'T'): {
                    if (info->apng_shared_insert_index == 0) {
                        info->apng_shared_insert_index = i;
                    }
                    if (first_frame_is_cover) {
                        kc_png_frame_info *frame = info->apng_frames + frame_index;
                        frame->chunk_num++;
                ...
                case KC_FOUR_CC('f', 'c', 'T', 'L'): {
                    ...
                    kc_png_chunk_fcTL_read(&frame->frame_control, data + chunk->offset + 8);
                } break;
                case KC_FOUR_CC('f', 'd', 'A', 'T'): {
                    kc_png_frame_info *frame = info->apng_frames + frame_index;
                    frame->chunk_num++;
...
/**
 ...
 Returns NULL if an error occurs.
 */
static uint8_t *kc_png_copy_frame_data_at_index(const uint8_t *data,
                                                const kc_png_info *info,
                                                const uint32_t index,
                                                uint32_t *size) {
    if (index >= info->apng_frame_num) return NULL;
    
    kc_png_frame_info *frame_info = info->apng_frames + index;
    ...
    *size = frame_remux_size;
    ...
    for (uint32_t i = 0; i < info->apng_shared_chunk_num; i++) {
        ...
        kc_png_chunk_info *shared_chunk_info = info->chunks + shared_chunk_index;
        ...
        if (shared_chunk_index >= info->apng_shared_insert_index && !inserted) { // replace IDAT with fdAT
            ...
            for (uint32_t c = 0; c < frame_info->chunk_num; c++) {
                kc_png_chunk_info *insert_chunk_info = info->chunks + frame_info->chunk_index + c;
                if (insert_chunk_info->fourcc == KC_FOUR_CC('f', 'd', 'A', 'T')) {
                    *((uint32_t *)(frame_data + data_offset)) = kc_swap_endian_uint32(insert_chunk_info->length - 4);
                    *((uint32_t *)(frame_data + data_offset + 4)) = KC_FOUR_CC('I', 'D', 'A', 'T');
                    ...
                    *((uint32_t *)(frame_data + data_offset + insert_chunk_info->length + 4)) = kc_swap_endian_uint32(crc);
        ...
        if (shared_chunk_info->fourcc == KC_FOUR_CC('I', 'H', 'D', 'R')) {
            uint8_t tmp[25] = {0};
            memcpy(tmp, data + shared_chunk_info->offset, 25);
            kc_png_chunk_IHDR IHDR = info->header;
            IHDR.width = frame_info->frame_control.width;
            IHDR.height = frame_info->frame_control.height;
            kc_png_chunk_IHDR_write(&IHDR, tmp + 8);
            *((uint32_t *)(tmp + 21)) = kc_swap_endian_uint32((uint32_t)crc32(0, tmp + 4, 17));
...
static inline size_t KCImageByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

/// Convert degree to radians
static inline CGFloat KCImageDegreesToRadians(CGFloat degrees) {
    return degrees * M_PI / 180;
}

CGColorSpaceRef KCCGColorSpaceGetDeviceRGB() {
...
CGColorSpaceRef KCCGColorSpaceGetDeviceGray() {
    static CGColorSpaceRef space;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
BOOL KCCGColorSpaceIsDeviceRGB(CGColorSpaceRef space) {
    return space && CFEqual(space, KCCGColorSpaceGetDeviceRGB());
}

BOOL KCCGColorSpaceIsDeviceGray(CGColorSpaceRef space) {
    return space && CFEqual(space, KCCGColorSpaceGetDeviceGray());
}



static void KCCGDataProviderReleaseDataCallback(void *info, const void *data, size_t size) {
    if (info) free(info);
}
 ...
 */
static BOOL KCCGImageDecodeToBitmapBufferWithAnyFormat(CGImageRef srcImage, vImage_Buffer *dest, vImage_CGImageFormat *destFormat) {
...
/**
 ...
 @return Whether succeed.
 */
static BOOL KCCGImageDecodeToBitmapBufferWith32BitFormat(CGImageRef srcImage, vImage_Buffer *dest, CGBitmapInfo bitmapInfo) {
    ...
    destFormat.colorSpace = KCCGColorSpaceGetDeviceRGB();
    destFormat.bitmapInfo = bitmapInfo;
    dest->data = NULL;
    if (KCCGImageDecodeToBitmapBufferWithAnyFormat(srcImage, dest, &destFormat)) return YES;
    ...
    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 32, KCCGColorSpaceGetDeviceRGB(), contextBitmapInfo);
...
CGImageRef KCCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay) {
    ...
    if (decodeForDisplay) { //decode with redraw (may lose some precision)
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, 0, KCCGColorSpaceGetDeviceRGB(), bitmapInfo);
...
CGImageRef KCCGImageCreateAffineTransformCopy(CGImageRef imageRef, CGAffineTransform transform, CGSize destSize, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    if(!KCCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &src, kCGImageAlphaFirst | kCGBitmapByteOrderDefault)) return NULL;
    
    size_t destBytesPerRow = KCImageByteAlign(destWidth * 4, 32);
    ...
    tmpProvider = CGDataProviderCreateWithData(tmp.data, tmp.data, destHeight * destBytesPerRow, KCCGDataProviderReleaseDataCallback);
    if (!tmpProvider) goto fail;
    tmp.data = NULL; // hold by provider
    tmpImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, KCCGColorSpaceGetDeviceRGB(), kCGImageAlphaFirst | kCGBitmapByteOrderDefault, tmpProvider, NULL, false, kCGRenderingIntentDefault);
    ...
    if (!KCCGImageDecodeToBitmapBufferWith32BitFormat(tmpImage, &dest, destBitmapInfo)) goto fail;
    CFRelease(tmpImage);
    tmpImage = NULL;
    
    destProvider = CGDataProviderCreateWithData(dest.data, dest.data, destHeight * destBytesPerRow, KCCGDataProviderReleaseDataCallback);
    if (!destProvider) goto fail;
    dest.data = NULL; // hold by provider
    destImage = CGImageCreate(destWidth, destHeight, 8, 32, destBytesPerRow, KCCGColorSpaceGetDeviceRGB(), destBitmapInfo, destProvider, NULL, false, kCGRenderingIntentDefault);
...
UIImageOrientation KCUIImageOrientationFromEXIFValue(NSInteger value) {
    switch (value) {
...
NSInteger KCUIImageOrientationToEXIFValue(UIImageOrientation orientation) {
    switch (orientation) {
...
CGImageRef KCCGImageCreateCopyWithOrientation(CGImageRef imageRef, UIImageOrientation orientation, CGBitmapInfo destBitmapInfo) {
    if (!imageRef) return NULL;
    ...
    switch (orientation) {
        case UIImageOrientationDown: {
            transform = CGAffineTransformMakeRotation(KCImageDegreesToRadians(180));
            transform = CGAffineTransformTranslate(transform, -(CGFloat)width, -(CGFloat)height);
        } break;
        case UIImageOrientationLeft: {
            transform = CGAffineTransformMakeRotation(KCImageDegreesToRadians(90));
    ...
    return KCCGImageCreateAffineTransformCopy(imageRef, transform, destSize, destBitmapInfo);
}

KCImageType KCImageDetectType(CFDataRef data) {
    if (!data) return KCImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return KCImageTypeUnknown;
    
    const char *bytes = (char *)CFDataGetBytePtr(data);
    
    uint32_t magic4 = *((uint32_t *)bytes);
    switch (magic4) {
        case KC_FOUR_CC(0x4D, 0x4D, 0x00, 0x2A): { // big endian TIFF
            return KCImageTypeTIFF;
        } break;
            
        case KC_FOUR_CC(0x49, 0x49, 0x2A, 0x00): { // little endian TIFF
            return KCImageTypeTIFF;
        } break;
            
        case KC_FOUR_CC(0x00, 0x00, 0x01, 0x00): { // ICO
            return KCImageTypeICO;
        } break;
            
        case KC_FOUR_CC(0x00, 0x00, 0x02, 0x00): { // CUR
            return KCImageTypeICO;
        } break;
            
        case KC_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return KCImageTypeICNS;
        } break;
            
        case KC_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return KCImageTypeGIF;
        } break;
            
        case KC_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == KC_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return KCImageTypePNG;
            }
        } break;
            
        case KC_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            uint32_t tmp = *((uint32_t *)(bytes + 8));
            if (tmp == KC_FOUR_CC('W', 'E', 'B', 'P')) {
                return KCImageTypeWebP;
            }
        } break;
    ...
    switch (magic2) {
        ...
        case KC_TWO_CC('P', 'I'):
        case KC_TWO_CC('C', 'I'):
        case KC_TWO_CC('C', 'P'): { // BMP
            return KCImageTypeBMP;
        }
        case KC_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return KCImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return KCImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return KCImageTypeJPEG2000;
    
    return KCImageTypeUnknown;
}

CFStringRef KCImageTypeToUTType(KCImageType type) {
    switch (type) {
        case KCImageTypeJPEG: return kUTTypeJPEG;
        case KCImageTypeJPEG2000: return kUTTypeJPEG2000;
        case KCImageTypeTIFF: return kUTTypeTIFF;
        case KCImageTypeBMP: return kUTTypeBMP;
        case KCImageTypeICO: return kUTTypeICO;
        case KCImageTypeICNS: return kUTTypeAppleICNS;
        case KCImageTypeGIF: return kUTTypeGIF;
        case KCImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

KCImageType KCImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(KCImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(KCImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(KCImageTypeTIFF),
                (id)kUTTypeBMP : @(KCImageTypeBMP),
                (id)kUTTypeICO : @(KCImageTypeICO),
                (id)kUTTypeAppleICNS : @(KCImageTypeICNS),
                (id)kUTTypeGIF : @(KCImageTypeGIF),
                (id)kUTTypePNG : @(KCImageTypePNG)};
    });
    if (!uti) return KCImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *KCImageTypeGetExtension(KCImageType type) {
    switch (type) {
        case KCImageTypeJPEG: return @"jpg";
        case KCImageTypeJPEG2000: return @"jp2";
        case KCImageTypeTIFF: return @"tiff";
        case KCImageTypeBMP: return @"bmp";
        case KCImageTypeICO: return @"ico";
        case KCImageTypeICNS: return @"icns";
        case KCImageTypeGIF: return @"gif";
        case KCImageTypePNG: return @"png";
        case KCImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef KCCGImageCreateEncodedData(CGImageRef imageRef, KCImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == KCImageTypeWebP) {
#if KCImage_WEBP_ENABLED
        if (quality == 1) {
            return KCCGImageCreateEncodedWebPData(imageRef, YES, quality, 4, KCImagePresetDefault);
        } else {
            return KCCGImageCreateEncodedWebPData(imageRef, NO, quality, 4, KCImagePresetDefault);
        }
#else
        return NULL;
#endif
    ...
    CFStringRef uti = KCImageTypeToUTType(type);
    if (!uti) return nil;
...
BOOL KCImageWebPAvailable() {
    return YES;
}

CFDataRef KCCGImageCreateEncodedWebPData(CGImageRef imageRef, BOOL lossless, CGFloat quality, int compressLevel, KCImagePreset preset) {
    if (!imageRef) return nil;
    size_t width = CGImageGetWidth(imageRef);
    size_t height = CGImageGetHeight(imageRef);
    ...
    if(!KCCGImageDecodeToBitmapBufferWith32BitFormat(imageRef, &buffer, kCGImageAlphaLast | kCGBitmapByteOrderDefault)) return nil;
    ...
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    preset = preset > KCImagePresetText ? KCImagePresetDefault : preset;
...
NSUInteger KCImageGetWebPFrameCount(CFDataRef webpData) {
...
CGImageRef KCCGImageCreateWithWebPData(CFDataRef webpData,
    ...
    bitsPerComponent = 8;
    bitsPerPixel = 32;
    bytesPerRow = KCImageByteAlign(bitsPerPixel / 8 * canvasWidth, 32);
    ...
    provider = CGDataProviderCreateWithData(destBytes, destBytes, destLength, KCCGDataProviderReleaseDataCallback);
    if (!provider) goto fail;
    destBytes = NULL; // hold by provider
    
    imageRef = CGImageCreate(canvasWidth, canvasHeight, bitsPerComponent, bitsPerPixel, bytesPerRow, KCCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
...
@implementation KCImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    KCImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    KCImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _KCImageDecoderFrame : KCImageFrame
...
@implementation _KCImageDecoderFrame
- (id)copyWithZone:(NSZone *)zone {
    _KCImageDecoderFrame *frame = [super copyWithZone:zone];
...
@implementation KCImageDecoder {
    pthread_mutex_t _lock;
    
    BOOL _sourceTypeDetected;
    CGImageSourceRef _source;
    kc_png_info *_apngSource;
#if KCImage_WEBP_ENABLED
...
- (void)dealloc {
    ...
    if (_apngSource) kc_png_info_release(_apngSource);
#if KCImage_WEBP_ENABLED
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    KCImageDecoder *decoder = [[KCImageDecoder alloc] initWithScale:scale];
...
- (KCImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    KCImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (NSTimeInterval)frameDurationAtIndex:(NSUInteger)index {
    ...
    if (index < _frames.count) {
        result = ((_KCImageDecoderFrame *)_frames[index]).duration;
    }
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    KCImageType type = KCImageDetectType((__bridge CFDataRef)data);
...
- (KCImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _KCImageDecoderFrame *frame = [(_KCImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != KCImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (!_needBlend) {
        ...
        if (decodeForDisplay && !decoded) {
            CGImageRef imageRefDecoded = KCCGImageCreateDecodedCopy(imageRef, YES);
            if (imageRefDecoded) {
                CFRelease(imageRef);
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == KCImageDisposeBackground) {
    ...
    if (extendToCanvas) {
        ...
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = KCImageDisposeNone;
        frame.blend = KCImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSourceWebP {
    ...
    if (WebPDemuxGetFrame(demuxer, 1, &iter)) { // one-based index...
        do {
            _KCImageDecoderFrame *frame = [_KCImageDecoderFrame new];
            [frames addObject:frame];
            if (iter.dispose_method == WEBP_MUX_DISPOSE_BACKGROUND) {
                frame.dispose = KCImageDisposeBackground;
            }
            if (iter.blend_method == WEBP_MUX_BLEND) {
                frame.blend = KCImageBlendOver;
            }
...
- (void)_updateSourceAPNG {
    ...
    kc_png_info_release(_apngSource);
    _apngSource = nil;
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        kc_png_frame_info *fi = apng->apng_frames + i;
        frame.index = i;
        frame.duration = kc_png_delay_to_seconds(fi->frame_control.delay_num, fi->frame_control.delay_den);
        frame.hasAlpha = YES;
        ...
        switch (fi->frame_control.dispose_op) {
            case KC_PNG_DISPOSE_OP_BACKGROUND: {
                frame.dispose = KCImageDisposeBackground;
            } break;
            case KC_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = KCImageDisposePrevious;
            } break;
            default: {
                frame.dispose = KCImageDisposeNone;
            } break;
        ...
        if (frame.blend == KCImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != KCImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == KCImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == KCImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == KCImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
        ...
        if (properties) {
            ...
            if (i == 0 && _width + _height == 0) { // init first frame
                ...
                if (value) {
                    ...
                    _orientation = KCUIImageOrientationFromEXIFValue(orientationValue);
                }
            }
            CFRelease(properties);
        }
...
- (CGImageRef)_newUnblendedImageAtIndex:(NSUInteger)index
    ...
    _KCImageDecoderFrame *frame = _frames[index];
    ...
    if (_source) {
        ...
        if (imageRef && extendToCanvas) {
            ...
            if (width == _width && height == _height) {
                CGImageRef imageRefExtended = KCCGImageCreateDecodedCopy(imageRef, YES);
                if (imageRefExtended) {
                    CFRelease(imageRef);
                ...
                CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, KCCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
                if (context) {
    ...
    if (_apngSource) {
        uint32_t size = 0;
        uint8_t *bytes = kc_png_copy_frame_data_at_index(_data.bytes, _apngSource, (uint32_t)index, &size);
        if (!bytes) return NULL;
        CGDataProviderRef provider = CGDataProviderCreateWithData(bytes, bytes, size, KCCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(bytes);
            return NULL;
        }
        ...
        if (!imageRef) return NULL;
        if (extendToCanvas) {
            CGContextRef context = CGBitmapContextCreate(NULL, _width, _height, 8, 0, KCCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst); //bgrA
            if (context) {
    ...
    if (_webpSource) {
        ...
        size_t bytesPerRow = KCImageByteAlign(bitsPerPixel / 8 * width, 32);
        ...
        CGDataProviderRef provider = CGDataProviderCreateWithData(pixels, pixels, length, KCCGDataProviderReleaseDataCallback);
        if (!provider) {
            free(pixels);
            return NULL;
        }
        ...
        CGImageRef image = CGImageCreate(width, height, bitsPerComponent, bitsPerPixel, bytesPerRow, KCCGColorSpaceGetDeviceRGB(), bitmapInfo, provider, NULL, false, kCGRenderingIntentDefault);
        CFRelease(provider);
        if (decoded) *decoded = YES;
        return image;
    }
...
- (BOOL)_createBlendContextIfNeeded {
    if (!_blendCanvas) {
        _blendFrameIndex = NSNotFound;
        _blendCanvas = CGBitmapContextCreate(NULL, _width, _height, 8, 0, KCCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    BOOL suc = _blendCanvas != NULL;
    return suc;
}

- (void)_blendImageWithFrame:(_KCImageDecoderFrame *)frame {
    if (frame.dispose == KCImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == KCImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == KCImageBlendOver) {
...
- (CGImageRef)_newBlendedImageWithFrame:(_KCImageDecoderFrame *)frame CF_RETURNS_RETAINED{
    CGImageRef imageRef = NULL;
    if (frame.dispose == KCImageDisposePrevious) {
        if (frame.blend == KCImageBlendOver) {
...
@implementation KCImageEncoder {
    NSMutableArray *_images;
    NSMutableArray *_durations;
}

- (instancetype)init {
    @throw [NSException exceptionWithName:@"KCImageEncoder init error" reason:@"KCImageEncoder must be initialized with a type. Use 'initWithType:' instead." userInfo:nil];
    return [self initWithType:KCImageTypeUnknown];
}

- (instancetype)initWithType:(KCImageType)type {
    if (type == KCImageTypeUnknown || type >= KCImageTypeOther) return nil;
    
#if !KCImage_WEBP_ENABLED
    if (type == KCImageTypeWebP) return nil;
#endif
    ...
    switch (type) {
        case KCImageTypeJPEG:
        case KCImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case KCImageTypeTIFF:
        case KCImageTypeBMP:
        case KCImageTypeGIF:
        case KCImageTypeICO:
        case KCImageTypeICNS:
        case KCImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, KCImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, KCImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (_type == KCImageTypeGIF && count > 1) {
...
- (CGImageRef)_newCGImageFromIndex:(NSUInteger)index decoded:(BOOL)decoded CF_RETURNS_RETAINED {
    ...
    if (image.imageOrientation != UIImageOrientationUp) {
        return KCCGImageCreateCopyWithOrientation(imageRef, image.imageOrientation, kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
    }
    if (decoded) {
        return KCCGImageCreateDecodedCopy(imageRef, YES);
    }
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == KCImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = KCCGImageCreateEncodedData(decoded, KCImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CGContextRef context = CGBitmapContextCreate(NULL, canvasWidth, canvasHeight, 8,
                                                     0, KCCGColorSpaceGetDeviceRGB(), kCGBitmapByteOrder32Host | kCGImageAlphaPremultipliedFirst);
        if (!context) {
            CFRelease(decoded);
            return nil;
        }
        ...
        CFDataRef frameData = KCCGImageCreateEncodedData(extendedImage, KCImageTypePNG, 1);
        if (!frameData) {
    ...
    kc_png_info *info = kc_png_info_create(firstFrameData.bytes, (uint32_t)firstFrameData.length);
    if (!info) return nil;
    ...
    png_header[0] = KC_FOUR_CC(0x89, 0x50, 0x4E, 0x47);
    png_header[1] = KC_FOUR_CC(0x0D, 0x0A, 0x1A, 0x0A);
    ...
    for (int i = 0; i < info->chunk_num; i++) {
        ...
        if (!insertBefore && chunk->fourcc == KC_FOUR_CC('I', 'D', 'A', 'T')) {
            insertBefore = YES;
            // insert acTL (APNG Control)
            uint32_t acTL[5] = {0};
            acTL[0] = kc_swap_endian_uint32(8); //length
            acTL[1] = KC_FOUR_CC('a', 'c', 'T', 'L'); // fourcc
            acTL[2] = kc_swap_endian_uint32((uint32_t)pngDatas.count); // num frames
            acTL[3] = kc_swap_endian_uint32((uint32_t)_loopCount); // num plays
            acTL[4] = kc_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(acTL + 1), 12)); //crc32
            [result appendBytes:acTL length:20];
            
            // insert fcTL (first frame control)
            kc_png_chunk_fcTL chunk_fcTL = {0};
            ...
            kc_png_delay_to_fraction([(NSNumber *)_durations[0] doubleValue], &chunk_fcTL.delay_num, &chunk_fcTL.delay_den);
            chunk_fcTL.delay_num = chunk_fcTL.delay_num;
            chunk_fcTL.delay_den = chunk_fcTL.delay_den;
            chunk_fcTL.dispose_op = KC_PNG_DISPOSE_OP_BACKGROUND;
            chunk_fcTL.blend_op = KC_PNG_BLEND_OP_SOURCE;
            
            uint8_t fcTL[38] = {0};
            *((uint32_t *)fcTL) = kc_swap_endian_uint32(26); //length
            *((uint32_t *)(fcTL + 4)) = KC_FOUR_CC('f', 'c', 'T', 'L'); // fourcc
            kc_png_chunk_fcTL_write(&chunk_fcTL, fcTL + 8);
            *((uint32_t *)(fcTL + 34)) = kc_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
            [result appendBytes:fcTL length:38];
            
            apngSequenceIndex++;
        }
        
        if (!insertAfter && insertBefore && chunk->fourcc != KC_FOUR_CC('I', 'D', 'A', 'T')) {
            insertAfter = YES;
            // insert fcTL and fdAT (APNG frame control and data)
            
            for (int i = 1; i < pngDatas.count; i++) {
                NSData *frameData = pngDatas[i];
                kc_png_info *frame = kc_png_info_create(frameData.bytes, (uint32_t)frameData.length);
                if (!frame) {
                    kc_png_info_release(info);
                    return nil;
                }
                ...
                *((uint32_t *)(fcTL + 34)) = kc_swap_endian_uint32((uint32_t)crc32(0, (const Bytef *)(fcTL + 4), 30));
                ...
                for (int d = 0; d < frame->chunk_num; d++) {
                    kc_png_chunk_info *dchunk = frame->chunks + d;
                    if (dchunk->fourcc == KC_FOUR_CC('I', 'D', 'A', 'T')) {
                        uint32_t length = kc_swap_endian_uint32(dchunk->length + 4);
                        [result appendBytes:&length length:4]; //length
                        uint32_t fourcc = KC_FOUR_CC('f', 'd', 'A', 'T');
                        [result appendBytes:&fourcc length:4]; //fourcc
                        uint32_t sq = kc_swap_endian_uint32(apngSequenceIndex);
                        ...
                        uint32_t crc = kc_swap_endian_uint32((uint32_t)crc32(0, bytes, dchunk->length + 8));
                        [result appendBytes:&crc length:4]; //crc
                        
                        apngSequenceIndex++;
                    }
                }
                kc_png_info_release(frame);
            }
        }
...
- (NSData *)_encodeWebP {
    ...
    for (NSUInteger i = 0; i < _images.count; i++) {
        ...
        if (!image) return nil;
        CFDataRef frameData = KCCGImageCreateEncodedWebPData(image, _lossless, _quality, 4, KCImagePresetDefault);
        CFRelease(image);
        if (!frameData) return nil;
...
- (NSData *)encode {
    ...
    if (_type == KCImageTypePNG) return [self _encodeAPNG];
    if (_type == KCImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(KCImageType)type quality:(CGFloat)quality {
    KCImageEncoder *encoder = [[KCImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(KCImageDecoder *)decoder type:(KCImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (KCImageCoder)

- (instancetype)imageByDecoded {
    if (self.isDecodedForDisplay) return self;
    CGImageRef imageRef = self.CGImage;
    if (!imageRef) return self;
    CGImageRef newImageRef = KCCGImageCreateDecodedCopy(imageRef, YES);
...
/// @param forSystem YES: used for system album (PNG/JPEG/GIF), NO: used for KCImage (PNG/JPEG/GIF/WebP)
- (NSData *)_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[KCImage class]]) {
        KCImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == KCImageTypeGIF ||
                    image.animatedImageType == KCImageTypePNG) {
    ...
    if (!data) {
        ...
        if (imageRef) {
            ...
            if (self.imageOrientation != UIImageOrientationUp) {
                CGImageRef rotated = KCCGImageCreateCopyWithOrientation(imageRef, self.imageOrientation, bitmapInfo | alphaInfo);
                if (rotated) {
                    CFRelease(imageRef);
                    imageRef = rotated;
                }
