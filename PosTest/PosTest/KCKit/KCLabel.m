...
static dispatch_queue_t KCLabelGetReleaseQueue() {
...
@interface KCLabel ()<KCTextDebugTarget,KCAsyncLayerDelegate>{
    
    NSMutableAttributedString *_innerText;
    KCTextLayout *_innerLayout;
    KCTextContainer *_innerContainer;
    NSMutableArray *_attachmentViews;
    NSMutableArray *_attachmentLayers;
    NSRange _highlightRange;
    KCTextHighlight *_highlight;
    KCTextLayout *_highlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [KCTextLayout layoutWithContainer:_innerContainer text:_innerText];
}
...
- (void)_clearInnerLayout {
    ...
    KCTextLayout *layout = _innerLayout;
    _innerLayout = nil;
    dispatch_async(KCLabelGetReleaseQueue(), ^{
...
- (void)_trackDidLongPress {
    ...
    if (_highlight) {
        KCTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
...
- (KCTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    KCTextHighlight *highlight = [_innerText attribute:KCTextHighlightAttributeName
                                               atIndex:startIndex
...
- (void)_updateOuterLineBreakMode {
    if (_innerContainer.truncationType) {
        switch (_innerContainer.truncationType) {
            case KCTextTruncationTypeStart: {
                _lineBreakMode = NSLineBreakByTruncatingHead;
            } break;
            case KCTextTruncationTypeEnd: {
                _lineBreakMode = NSLineBreakByTruncatingTail;
            } break;
            case KCTextTruncationTypeMiddle: {
...
- (void)_initLabel {
    ((KCAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _debugOption = [KCTextDebugOption sharedDebugOption];
    [KCTextDebugOption addDebugTarget:self];
    
    _textVerticalAlignment = KCTextVerticalAlignmentCenter;
    _numberOfLines = 1;
    _lineBreakMode = NSLineBreakByTruncatingTail;
    _innerText = [NSMutableAttributedString new];
    _innerContainer = [KCTextContainer new];
    _innerContainer.truncationType = KCTextTruncationTypeEnd;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    [self _initLabel];
    KCTextContainer *innerContainer = [aDecoder decodeObjectForKey:@"innerContainer"];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved) {
            KCTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
            KCTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
            if (tapAction) {
                CGRect rect = [_innerLayout rectForRange:[KCTextRange rangeWithRange:_highlightRange]];
                rect = [self _convertRectFromLayout:rect];
...
- (void)setLineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    switch (lineBreakMode) {
        ...
        case NSLineBreakByCharWrapping:
        case NSLineBreakByClipping: {
            _innerContainer.truncationType = KCTextTruncationTypeNone;
            _innerText.lineBreakMode = lineBreakMode;
        } break;
        case NSLineBreakByTruncatingHead:{
            _innerContainer.truncationType = KCTextTruncationTypeStart;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        case NSLineBreakByTruncatingTail:{
            _innerContainer.truncationType = KCTextTruncationTypeEnd;
        ...
        case NSLineBreakByTruncatingMiddle: {
            _innerContainer.truncationType = KCTextTruncationTypeMiddle;
            _innerText.lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        default: break;
    }
...
- (void)setTextLayout:(KCTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((KCAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (KCAsyncLayerDisplayTask *)newAsyncDisplayTask {
    ...
    KCTextContainer *container = _innerContainer;
    KCTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    KCTextDebugOption *debug = _debugOption;
    ...
    __block KCTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? _highlightLayout : _innerLayout;
    ...
    // create display task
    KCAsyncLayerDisplayTask *task = [KCAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        if (layoutNeedUpdate) {
            layout = [KCTextLayout layoutWithContainer:container text:text];
            if (isCancelled()) return;
            layoutUpdated = YES;
        }
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        if (!finished) {
            ...
            for (KCTextAttachment *a in layout.attachments) {
