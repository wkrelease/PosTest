...
KCSYNTH_DUMMY_CLASS(UIDevice_KCAdd)

@implementation UIDevice (KCAdd)
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} kc_net_interface_counter;


static uint64_t kc_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t kc_net_counter_get_by_type(kc_net_interface_counter *counter, KCNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & KCNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & KCNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & KCNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & KCNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & KCNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & KCNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static kc_net_interface_counter kc_get_net_interface_counter() {
    ...
    kc_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = kc_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = kc_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(KCNetworkTrafficType)types {
    kc_net_interface_counter counter = kc_get_net_interface_counter();
    return kc_net_counter_get_by_type(&counter, types);
}
