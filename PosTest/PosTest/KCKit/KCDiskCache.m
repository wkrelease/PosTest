...
static int64_t _KCDiskSpaceFree() {
    NSError *error = nil;
...
- (void)_trimToFreeDiskSpace:(NSUInteger)targetFreeDiskSpace {
    ...
    int64_t diskFreeBytes = _KCDiskSpaceFree();
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"KCDiskCache init error" reason:@"KCDiskCache must be initialized with a path. Use 'initWithPath:' or 'initWithPath:inlineThreshold:' instead." userInfo:nil];
...
- (instancetype)initWithPath:(NSString *)path
    ...
    KCKVStorageType type;
    if (threshold == 0) {
        type = KCKVStorageTypeFile;
    } else if (threshold == NSUIntegerMax) {
        type = KCKVStorageTypeSQLite;
    } else {
        type = KCKVStorageTypeMixed;
    }
    
    KCKVStorage *kv = [[KCKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    KCKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [KCDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [KCDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != KCKVStorageTypeSQLite) {
