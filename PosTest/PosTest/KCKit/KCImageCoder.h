...
/**
 Image file type.
 */
typedef NS_ENUM(NSUInteger, KCImageType) {
    KCImageTypeUnknown = 0, ///< unknown
    KCImageTypeJPEG,        ///< jpeg, jpg
    KCImageTypeJPEG2000,    ///< jp2
    KCImageTypeTIFF,        ///< tiff, tif
    KCImageTypeBMP,         ///< bmp
    KCImageTypeICO,         ///< ico
    KCImageTypeICNS,        ///< icns
    KCImageTypeGIF,         ///< gif
    KCImageTypePNG,         ///< png
    KCImageTypeWebP,        ///< webp
    KCImageTypeOther,       ///< other image format
};
 ...
 */
typedef NS_ENUM(NSUInteger, KCImageBlendOperation) {
 ...
 */
@interface KCImageFrame : NSObject <NSCopying>
...
@property (nonatomic, assign) KCImageDisposeMethod dispose; ///< Frame dispose method.
@property (nonatomic, assign) KCImageBlendOperation blend;  ///< Frame blend operation.
...
/**
 ...
 Example:
    ...
    KCImageDecoder *decoder = [KCImageDecoder decoderWithData:data scale:2.0];
    UIImage image = [decoder frameAtIndex:0 decodeForDisplay:YES].image;
 
    // Decode image during download:
    NSMutableData *data = [NSMutableData new];
    KCImageDecoder *decoder = [[KCImageDecoder alloc] initWithScale:2.0];
 ...
 */
@interface KCImageDecoder : NSObject

@property (nonatomic, readonly) NSData *data;       ///< Image data.
@property (nonatomic, readonly) KCImageType type;   ///< Image data type.
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in KCImageType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    KCImageEncoder *jpegEncoder = [[KCImageEncoder alloc] initWithType:KCImageTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    KCImageEncoder *gifEncoder = [[KCImageEncoder alloc] initWithType:KCImageTypeGIF];
 ...
 */
@interface KCImageEncoder : NSObject

@property (nonatomic, readonly) KCImageType type;   ///< Image type.
 ...
 */
- (instancetype)initWithType:(KCImageType)type NS_DESIGNATED_INITIALIZER;
...
@interface UIImage (KCImageCoder)
...
CG_EXTERN KCImageType KCImageDetectType(CFDataRef data);

/// Convert KCImageType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef KCImageTypeToUTType(KCImageType type);

/// Convert UTI (such as kUTTypeJPEG) to KCImageType.
CG_EXTERN KCImageType KCImageTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *KCImageTypeGetExtension(KCImageType type);



/// Returns the shared DeviceRGB color space.
CG_EXTERN CGColorSpaceRef KCCGColorSpaceGetDeviceRGB();

/// Returns the shared DeviceGray color space.
CG_EXTERN CGColorSpaceRef KCCGColorSpaceGetDeviceGray();

/// Returns whether a color space is DeviceRGB.
CG_EXTERN BOOL KCCGColorSpaceIsDeviceRGB(CGColorSpaceRef space);

/// Returns whether a color space is DeviceGray.
CG_EXTERN BOOL KCCGColorSpaceIsDeviceGray(CGColorSpaceRef space);



/// Convert EXIF orientation value to UIImageOrientation.
CG_EXTERN UIImageOrientation KCUIImageOrientationFromEXIFValue(NSInteger value);

/// Convert UIImageOrientation to EXIF orientation value.
CG_EXTERN NSInteger KCUIImageOrientationToEXIFValue(UIImageOrientation orientation);
 ...
 */
CG_EXTERN CGImageRef KCCGImageCreateDecodedCopy(CGImageRef imageRef, BOOL decodeForDisplay);
 ...
 */
CG_EXTERN CGImageRef KCCGImageCreateCopyWithOrientation(CGImageRef imageRef,
...
/**
 ...
 @return A new image, or NULL if an error occurs.
 */
CG_EXTERN CGImageRef KCCGImageCreateAffineTransformCopy(CGImageRef imageRef,
 ...
 */
CG_EXTERN CFDataRef KCCGImageCreateEncodedData(CGImageRef imageRef, KCImageType type, CGFloat quality);


/**
 Whether WebP is available in KCImage.
 */
CG_EXTERN BOOL KCImageWebPAvailable();
 ...
 */
CG_EXTERN NSUInteger KCImageGetWebPFrameCount(CFDataRef webpData);
 ...
 */
CG_EXTERN CGImageRef KCCGImageCreateWithWebPData(CFDataRef webpData,
...
typedef NS_ENUM(NSUInteger, KCImagePreset) {
    KCImagePresetDefault = 0,  ///< default preset.
    KCImagePresetPicture,      ///< digital picture, like portrait, inner shot
    KCImagePresetPhoto,        ///< outdoor photograph, with natural lighting
    KCImagePresetDrawing,      ///< hand or line drawing, with high-contrast details
    KCImagePresetIcon,         ///< small-sized colorful images
    KCImagePresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is KCImagePresetDefault.
 @return WebP data, or nil if an error occurs.
 */
CG_EXTERN CFDataRef KCCGImageCreateEncodedWebPData(CGImageRef imageRef,
                                         BOOL lossless,
                                         CGFloat quality,
                                         int compressLevel,
                                         KCImagePreset preset);
