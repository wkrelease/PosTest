...
KCSYNTH_DUMMY_CLASS(UIImageView_KCWebImage)

static int _KCWebImageSetterKey;
static int _KCWebImageHighlightedSetterKey;

@implementation UIImageView (KCWebImage)
...
- (NSURL *)highlightedImageURL {
    _KCWebImageSetter *setter = objc_getAssociatedObject(self, &_KCWebImageHighlightedSetterKey);
    return setter.imageURL;
}
...
- (void)setHighlightedImageWithURL:(NSURL *)imageURL
    ...
    if (!setter) {
        setter = [_KCWebImageSetter new];
        objc_setAssociatedObject(self, &_KCWebImageHighlightedSetterKey, setter, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    int32_t sentinel = [setter cancelWithNewURL:imageURL];
