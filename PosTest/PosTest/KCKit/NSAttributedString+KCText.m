...
#import "NSParagraphStyle+KCText.h"
...
KCSYNTH_DUMMY_CLASS(NSAttributedString_KCText)

@implementation NSAttributedString (KCText)


- (NSData *)archiveToData {
    NSData *data = nil;
    @try {
        data = [KCTextArchiver archivedDataWithRootObject:self];
    }
...
+ (instancetype)unarchiveFromData:(NSData *)data {
    ...
    @try {
        one = [KCTextUnarchiver unarchiveObjectWithData:data];
    }
    @catch (NSException *exception) {
        NSLog(@"%@",exception);
    }
...
- (KCTextShadow *)textShadow {
    return [self textShadowAtIndex:0];
}

- (KCTextShadow *)textShadowAtIndex:(NSUInteger)index {
    return [self attribute:KCTextShadowAttributeName atIndex:index];
}

- (KCTextShadow *)textInnerShadow {
    return [self textInnerShadowAtIndex:0];
}

- (KCTextShadow *)textInnerShadowAtIndex:(NSUInteger)index {
    return [self attribute:KCTextInnerShadowAttributeName atIndex:index];
}

- (KCTextDecoration *)textUnderline {
    return [self textUnderlineAtIndex:0];
}

- (KCTextDecoration *)textUnderlineAtIndex:(NSUInteger)index {
    return [self attribute:KCTextUnderlineAttributeName atIndex:index];
}

- (KCTextDecoration *)textStrikethrough {
    return [self textStrikethroughAtIndex:0];
}

- (KCTextDecoration *)textStrikethroughAtIndex:(NSUInteger)index {
    return [self attribute:KCTextStrikethroughAttributeName atIndex:index];
}

- (KCTextBorder *)textBorder {
    return [self textBorderAtIndex:0];
}

- (KCTextBorder *)textBorderAtIndex:(NSUInteger)index {
    return [self attribute:KCTextBorderAttributeName atIndex:index];
}

- (KCTextBorder *)textBackgroundBorder {
    return [self textBackgroundBorderAtIndex:0];
}

- (KCTextBorder *)textBackgroundBorderAtIndex:(NSUInteger)index {
    return [self attribute:KCTextBackedStringAttributeName atIndex:index];
}
...
- (CGAffineTransform)textGlyphTransformAtIndex:(NSUInteger)index {
    NSValue *value = [self attribute:KCTextGlyphTransformAttributeName atIndex:index];
...
- (NSString *)plainTextForRange:(NSRange)range {
    ...
    [self enumerateAttribute:KCTextBackedStringAttributeName inRange:range options:kNilOptions usingBlock:^(id value, NSRange range, BOOL *stop) {
        KCTextBackedString *backed = value;
...
+ (NSMutableAttributedString *)attachmentStringWithContent:(id)content
    ...
    NSMutableAttributedString *atr = [[NSMutableAttributedString alloc] initWithString:KCTextAttachmentToken];
    
    KCTextAttachment *attach = [KCTextAttachment new];
...
+ (NSMutableAttributedString *)attachmentStringWithContent:(id)content
    ...
    switch (alignment) {
        case KCTextVerticalAlignmentTop: {
...
+ (NSMutableAttributedString *)attachmentStringWithEmojiImage:(UIImage *)image
    ...
    if (image.images.count > 1) {
        hasAnim = YES;
    } else if ([image conformsToProtocol:@protocol(KCAnimatedImage)]) {
        id <KCAnimatedImage> ani = (id)image;
        if (ani.animatedImageFrameCount > 1) hasAnim = YES;
    }
    
    CGFloat ascent = KCEmojiGetAscentWithFontSize(fontSize);
    CGFloat descent = KCEmojiGetDescentWithFontSize(fontSize);
    CGRect bounding = KCEmojiGetGlyphBoundingRectWithFontSize(fontSize);
    ...
    KCTextAttachment *attachment = [KCTextAttachment new];
    ...
    if (hasAnim) {
        KCAnimatedImageView *view = [KCAnimatedImageView new];
        view.frame = bounding;
        view.image = image;
...
- (BOOL)canDrawWithUIKit {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [failSet addObject:KCTextShadowAttributeName];
        [failSet addObject:KCTextInnerShadowAttributeName];
        [failSet addObject:KCTextUnderlineAttributeName];
        [failSet addObject:KCTextStrikethroughAttributeName];
        [failSet addObject:KCTextBorderAttributeName];
        [failSet addObject:KCTextBackgroundBorderAttributeName];
        [failSet addObject:KCTextBlockBorderAttributeName];
        [failSet addObject:KCTextAttachmentAttributeName];
        [failSet addObject:KCTextHighlightAttributeName];
        [failSet addObject:KCTextGlyphTransformAttributeName];
    });
...
@implementation NSMutableAttributedString (KCText)
...
- (void)setTextBackedString:(KCTextBackedString *)textBackedString range:(NSRange)range {
    [self setAttribute:KCTextBackedStringAttributeName value:textBackedString range:range];
}

- (void)setTextBinding:(KCTextBinding *)textBinding range:(NSRange)range {
    [self setAttribute:KCTextBindingAttributeName value:textBinding range:range];
}

- (void)setTextShadow:(KCTextShadow *)textShadow range:(NSRange)range {
    [self setAttribute:KCTextShadowAttributeName value:textShadow range:range];
}

- (void)setTextInnerShadow:(KCTextShadow *)textInnerShadow range:(NSRange)range {
    [self setAttribute:KCTextInnerShadowAttributeName value:textInnerShadow range:range];
}

- (void)setTextUnderline:(KCTextDecoration *)textUnderline range:(NSRange)range {
    [self setAttribute:KCTextUnderlineAttributeName value:textUnderline range:range];
}

- (void)setTextStrikethrough:(KCTextDecoration *)textStrikethrough range:(NSRange)range {
    [self setAttribute:KCTextStrikethroughAttributeName value:textStrikethrough range:range];
}

- (void)setTextBorder:(KCTextBorder *)textBorder range:(NSRange)range {
    [self setAttribute:KCTextBorderAttributeName value:textBorder range:range];
}

- (void)setTextBackgroundBorder:(KCTextBorder *)textBackgroundBorder range:(NSRange)range {
    [self setAttribute:KCTextBackgroundBorderAttributeName value:textBackgroundBorder range:range];
}

- (void)setTextAttachment:(KCTextAttachment *)textAttachment range:(NSRange)range {
    [self setAttribute:KCTextAttachmentAttributeName value:textAttachment range:range];
}

- (void)setTextHighlight:(KCTextHighlight *)textHighlight range:(NSRange)range {
    [self setAttribute:KCTextHighlightAttributeName value:textHighlight range:range];
}

- (void)setTextBlockBorder:(KCTextBorder *)textBlockBorder range:(NSRange)range {
    [self setAttribute:KCTextBlockBorderAttributeName value:textBlockBorder range:range];
}

- (void)setTextRubyAnnotation:(KCTextRubyAnnotation *)ruby range:(NSRange)range {
    if (kiOS8Later) {
...
- (void)setTextGlyphTransform:(CGAffineTransform)textGlyphTransform range:(NSRange)range {
    ...
    [self setAttribute:KCTextGlyphTransformAttributeName value:value range:range];
}
...
+ (NSArray *)allDiscontinuousAttributeKeys {
    ...
    dispatch_once(&onceToken, ^{
        keys = @[(id)kCTSuperscriptAttributeName,
                 ...
                 KCTextBackedStringAttributeName,
                 KCTextBindingAttributeName,
                 KCTextAttachmentAttributeName].mutableCopy;
        if (kiOS8Later) {
