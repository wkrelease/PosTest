...
typedef NS_ENUM(NSUInteger, KCAnimagedImageType) {
    KCAnimagedImageTypeNone = 0,
    KCAnimagedImageTypeImage,
    KCAnimagedImageTypeHighlightedImage,
    KCAnimagedImageTypeImages,
    KCAnimagedImageTypeHighlightedImages,
};

@interface KCAnimatedImageView() {
    @package
    UIImage <KCAnimatedImage> *_curAnimatedImage;
...
@interface _KCAnimatedImageViewFetchOperation : NSOperation
@property (nonatomic, weak) KCAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <KCAnimatedImage> *curImage;
@end

@implementation _KCAnimatedImageViewFetchOperation
- (void)main {
    __strong KCAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation KCAnimatedImageView
...
- (void)resetAnimated {
    dispatch_once(&_onceToken, ^{
        ...
        _link = [CADisplayLink displayLinkWithTarget:[KCWeakProxy proxyWithTarget:self] selector:@selector(step:)];
        if (_runloopMode) {
...
- (void)setImage:(UIImage *)image {
    ...
    [self setImage:image withType:KCAnimagedImageTypeImage];
}

- (void)setHighlightedImage:(UIImage *)highlightedImage {
    if (self.highlightedImage == highlightedImage) return;
    [self setImage:highlightedImage withType:KCAnimagedImageTypeHighlightedImage];
}

- (void)setAnimationImages:(NSArray *)animationImages {
    if (self.animationImages == animationImages) return;
    [self setImage:animationImages withType:KCAnimagedImageTypeImages];
}
...
- (void)setHighlightedAnimationImages:(NSArray *)highlightedAnimationImages {
    ...
    [self setImage:highlightedAnimationImages withType:KCAnimagedImageTypeHighlightedImages];
}
...
- (id)imageForType:(KCAnimagedImageType)type {
    switch (type) {
        case KCAnimagedImageTypeNone: return nil;
        case KCAnimagedImageTypeImage: return self.image;
        case KCAnimagedImageTypeHighlightedImage: return self.highlightedImage;
        case KCAnimagedImageTypeImages: return self.animationImages;
        case KCAnimagedImageTypeHighlightedImages: return self.highlightedAnimationImages;
    }
    return nil;
}

- (KCAnimagedImageType)currentImageType {
    KCAnimagedImageType curType = KCAnimagedImageTypeNone;
    if (self.highlighted) {
        if (self.highlightedAnimationImages.count) curType = KCAnimagedImageTypeHighlightedImages;
        else if (self.highlightedImage) curType = KCAnimagedImageTypeHighlightedImage;
    }
    if (curType == KCAnimagedImageTypeNone) {
        if (self.animationImages.count) curType = KCAnimagedImageTypeImages;
        else if (self.image) curType = KCAnimagedImageTypeImage;
    }
    return curType;
}

- (void)setImage:(id)image withType:(KCAnimagedImageType)type {
    [self stopAnimating];
    if (_link) [self resetAnimated];
    _curFrame = nil;
    switch (type) {
        case KCAnimagedImageTypeNone: break;
        case KCAnimagedImageTypeImage: super.image = image; break;
        case KCAnimagedImageTypeHighlightedImage: super.highlightedImage = image; break;
        case KCAnimagedImageTypeImages: super.animationImages = image; break;
        case KCAnimagedImageTypeHighlightedImages: super.highlightedAnimationImages = image; break;
    }
    [self imageChanged];
}

- (void)imageChanged {
    KCAnimagedImageType newType = [self currentImageType];
    id newVisibleImage = [self imageForType:newType];
    NSUInteger newImageFrameCount = 0;
    BOOL hasContentsRect = NO;
    if ([newVisibleImage isKindOfClass:[UIImage class]] &&
        [newVisibleImage conformsToProtocol:@protocol(KCAnimatedImage)]) {
        newImageFrameCount = ((UIImage<KCAnimatedImage> *) newVisibleImage).animatedImageFrameCount;
        if (newImageFrameCount > 1) {
            hasContentsRect = [((UIImage<KCAnimatedImage> *) newVisibleImage) respondsToSelector:@selector(animatedImageContentsRectAtIndex:)];
        }
    }
    ...
    if (hasContentsRect) {
        CGRect rect = [((UIImage<KCAnimatedImage> *) newVisibleImage) animatedImageContentsRectAtIndex:0];
...
- (void)startAnimating {
    KCAnimagedImageType type = [self currentImageType];
    if (type == KCAnimagedImageTypeImages || type == KCAnimagedImageTypeHighlightedImages) {
...
- (void)step:(CADisplayLink *)link {
    UIImage <KCAnimatedImage> *image = _curAnimatedImage;
    ...
    if (!bufferIsFull && _requestQueue.operationCount == 0) { // if some work not finished, wait for next opportunity
        _KCAnimatedImageViewFetchOperation *operation = [_KCAnimatedImageViewFetchOperation new];
        operation.view = self;
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    UIImage *image = [aDecoder decodeObjectForKey:@"KCAnimatedImage"];
    UIImage *highlightedImage = [aDecoder decodeObjectForKey:@"KCHighlightedAnimatedImage"];
    if (image) {
        self.image = image;
        [self setImage:image withType:KCAnimagedImageTypeImage];
    }
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    BOOL ani, multi;
    ani = [self.image conformsToProtocol:@protocol(KCAnimatedImage)];
    multi = (ani && ((UIImage <KCAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.image forKey:@"KCAnimatedImage"];
    
    ani = [self.highlightedImage conformsToProtocol:@protocol(KCAnimatedImage)];
    multi = (ani && ((UIImage <KCAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.highlightedImage forKey:@"KCHighlightedAnimatedImage"];
}
