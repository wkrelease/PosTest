...
#import "KCTextUtilities.h"
#import "KCTextAttribute.h"
#import <libkern/OSAtomic.h>
#import "NSAttributedString+KCText.h"
#import "UIFont+KCAdd.h"

const CGSize KCTextContainerMaxSize = (CGSize){0x100000, 0x100000};
...
static inline CGSize KCTextClipCGSize(CGSize size) {
    if (size.width > KCTextContainerMaxSize.width) size.width = KCTextContainerMaxSize.width;
    if (size.height > KCTextContainerMaxSize.height) size.height = KCTextContainerMaxSize.height;
    return size;
}
...
- (id)copyWithZone:(NSZone *)zone {
    KCTextLinePositionSimpleModifier *one = [self.class new];
...
@implementation KCTextContainer {
    @package
    BOOL _readonly; ///< used only in KCTextLayout.implementation
    OSSpinLock _lock;
    ...
    KCTextTruncationType _truncationType;
    NSAttributedString *_truncationToken;
    id<KCTextLinePositionModifier> _linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    KCTextContainer *one = [self new];
    one.size = KCTextClipCGSize(size);
    one.insets = insets;
    return one;
}
...
- (id)copyWithZone:(NSZone *)zone {
    KCTextContainer *one = [self.class new];
...
reason:@"Cannot change the property of the 'container' in 'KCTextLayout'." userInfo:nil]; \
return; \
} \
OSSpinLockLock(&_lock); \
__VA_ARGS__; \
...
- (void)setSize:(CGSize)size {
    Setter(if(!_path) _size = KCTextClipCGSize(size));
}
...
- (KCTextTruncationType)truncationType {
    Getter(KCTextTruncationType type = _truncationType) return type;
}

- (void)setTruncationType:(KCTextTruncationType)truncationType {
...
- (void)setLinePositionModifier:(id<KCTextLinePositionModifier>)linePositionModifier {
    Setter(_linePositionModifier = [(NSObject *)linePositionModifier copy]);
}

- (id<KCTextLinePositionModifier>)linePositionModifier {
    Getter(id<KCTextLinePositionModifier> m = _linePositionModifier) return m;
}
...
@interface KCTextLayout ()

@property (nonatomic, readwrite) KCTextContainer *container;
...
@property (nonatomic, readwrite) KCTextLine *truncatedLine;
...
@property (nonatomic, assign) KCRowEdge *lineRowsEdge; ///< top-left origin

@end



@implementation KCTextLayout
...
+ (KCTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
    KCTextContainer *container = [KCTextContainer containerWithSize:size];
...
+ (KCTextLayout *)layoutWithContainer:(KCTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    ...
    KCTextLine *truncatedLine = nil;
    KCRowEdge *lineRowsEdge = NULL;
    ...
    layout = [[KCTextLayout alloc] _init];
    layout.text = text;
    ...
    ctFrame = CTFramesetterCreateFrame(ctSetter, KCCFRangeFromNSRange(range), cgPath, (CFTypeRef)frameAttrs);
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        KCTextLine *line = [KCTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    KCTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        KCTextLine *lastLine = lines.lastObject;
        ...
        lineRowsEdge = calloc(rowCount, sizeof(KCRowEdge));
        ...
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            ...
            if ((NSInteger)line.row != lastRowIdx) {
                if (lastRowIdx >= 0) {
                    lineRowsEdge[lastRowIdx] = (KCRowEdge) {.head = lastHead, .foot = lastFoot };
                }
                lastRowIdx = line.row;
        ...
        for (NSUInteger i = 1; i < rowCount; i++) {
            KCRowEdge v0 = lineRowsEdge[i - 1];
            KCRowEdge v1 = lineRowsEdge[i];
    ...
    visibleRange = KCNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
    if (needTruncation) {
        KCTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != KCTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                if (runCount > 0) {
                    ...
                    [attrs removeObjectForKey:KCTextAttachmentAttributeName];
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:KCTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == KCTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                } else if (container.truncationType == KCTextTruncationTypeMiddle) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [KCTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = KCTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = KCTextVerticalFormRotateAndMoveCharacterSet();
        
        void (^lineBlock)(KCTextLine *) = ^(KCTextLine *line){
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                KCTextRunGlyphDrawMode prevMode = KCTextRunGlyphDrawModeHorizontal;
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    KCTextRunGlyphDrawMode mode = glyphRotateMove ? KCTextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? KCTextRunGlyphDrawModeVerticalRotate : KCTextRunGlyphDrawModeHorizontal);
                    if (g == 0) {
                        prevMode = mode;
                    } else if (mode != prevMode) {
                        KCTextRunGlyphRange *aRange = [KCTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    KCTextRunGlyphRange *aRange = [KCTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
                
            }
        };
        for (KCTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[KCTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[KCTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[KCTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[KCTextShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[KCTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[KCTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[KCTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[KCTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[KCTextBorderAttributeName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (KCTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        KCTextContainer *container = containers[i];
        KCTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (NSUInteger)_rowIndexForEdge:(CGFloat)edge {
    ...
    while (lo <= hi) {
        mid = (lo + hi) / 2;
        KCRowEdge oneEdge = _lineRowsEdge[mid];
        if (isVertical ?
 ...
 */
- (CTRunRef)_runForLine:(KCTextLine *)line position:(KCTextPosition *)position {
    if (!line || !position) return NULL;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
        CTRunRef run = CFArrayGetValueAtIndex(runs, i);
        CFRange range = CTRunGetStringRange(run);
        if (position.affinity == KCTextAffinityBackward) {
...
/**
 ...
 next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(KCTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(KCTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(KCTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((KCTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        CGRect bounds = ((KCTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    KCTextLine *line = _lines[lineIndex];
...
- (KCTextPosition *)closestPositionToPoint:(CGPoint)point {
    ...
    if (position <= _visibleRange.location) {
        return [KCTextPosition positionWithOffset:_visibleRange.location affinity:KCTextAffinityForward];
    } else if (position >= _visibleRange.location + _visibleRange.length) {
        return [KCTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:KCTextAffinityBackward];
    }
    
    KCTextAffinity finalAffinity = KCTextAffinityForward;
    BOOL finalAffinityDetected = NO;
    
    // binding range
    NSRange bindingRange;
    KCTextBinding *binding = [_text attribute:KCTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[KCTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[KCTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:KCTextAffinityBackward]];
            ...
            if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
                if (_container.isVerticalForm) {
                    if (fabs(point.y - left) < fabs(point.y - right)) {
                        ...
                        finalAffinity = KCTextAffinityForward;
                    } else {
                        position = bindingRange.location + bindingRange.length;
                        finalAffinity = KCTextAffinityBackward;
                    }
                } else {
    ...
    if (line.range.length == 0) {
        ...
        return [KCTextPosition positionWithOffset:line.range.location affinity:behind ? KCTextAffinityBackward:KCTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (KCTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [KCTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (KCTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (KCTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
    ...
    if (position == line.range.location) {
        return [KCTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [KCTextPosition positionWithOffset:position affinity:KCTextAffinityBackward];
    }
    ...
    if (!finalAffinityDetected) {
        ...
        if (ofs != CGFLOAT_MAX) {
            ...
            if (position >= line.range.location + line.range.length) {
                finalAffinity = RTL ? KCTextAffinityForward : KCTextAffinityBackward;
            } else if (position <= line.range.location) {
                finalAffinity = RTL ? KCTextAffinityBackward : KCTextAffinityForward;
            } else {
                finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? KCTextAffinityForward : KCTextAffinityBackward;
            }
        }
    }
    
    return [KCTextPosition positionWithOffset:position affinity:finalAffinity];
}
...
- (KCTextPosition *)positionForPoint:(CGPoint)point
    ...
    KCTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    KCTextLine *line = _lines[lineIndex];
    KCRowEdge vertical = _lineRowsEdge[line.row];
    if (_container.verticalForm) {
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            KCTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        } else { // search forward
            KCTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    } else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            KCTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        } else { // search forward
            KCTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (KCTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    KCTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        KCTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp:UITextLayoutDirectionDown offset:1];
        return range;
    } else {
        KCTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft:UITextLayoutDirectionRight offset:1];
        return range;
    }
}
...
- (KCTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    ...
    KCTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}
...
- (KCTextRange *)textRangeByExtendingPosition:(KCTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [KCTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    } else if (position.offset == visibleEnd) {
        return [KCTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:KCTextAffinityBackward];
    }
    
    // binding range
    NSRange tRange;
    KCTextBinding *binding = [_text attribute:KCTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [KCTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [KCTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [KCTextRange rangeWithStart:[KCTextPosition positionWithOffset:position.offset - 1] end:[KCTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (KCTextIsLinebreakChar(c0) && position.affinity == KCTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = KCTextLinebreakTailLength(str);
            return [KCTextRange rangeWithStart:[KCTextPosition positionWithOffset:position.offset - len] end:[KCTextPosition positionWithOffset:position.offset]];
        }
    }
    
    return [KCTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (KCTextRange *)textRangeByExtendingPosition:(KCTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [KCTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    } else if (forwardMove && position.offset == visibleEnd) {
        return [KCTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:KCTextAffinityBackward];
    }
    
    // extend from position
    KCTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    KCTextRange *allForward = [KCTextRange rangeWithStart:fromRange.start end:[KCTextPosition positionWithOffset:visibleEnd]];
    KCTextRange *allBackward = [KCTextRange rangeWithStart:[KCTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        KCTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        KCTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        KCTextPosition *extPos;
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string substringWithRange:insideLine.range];
                NSUInteger lineBreakLen = KCTextLinebreakTailLength(subStr);
                extPos = [KCTextPosition positionWithOffset:pos - lineBreakLen];
            } else {
                extPos = [KCTextPosition positionWithOffset:pos];
            }
        } else {
            extPos = [KCTextPosition positionWithOffset:pos];
        }
        KCTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [KCTextRange rangeWithStart:fromRange.start end:ext.end];
        } else {
            return [KCTextRange rangeWithStart:ext.start end:fromRange.end];
        }
        
    } else { // left/right in text layout
        KCTextPosition *toPosition = [KCTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart) return allBackward;
        else if (toPosition.offset >= visibleEnd) return allForward;
        
        KCTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [KCTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}
...
- (CGRect)firstRectForRange:(KCTextRange *)range {
    ...
    KCTextLine *startLine = _lines[startLineIndex];
    KCTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        KCTextLine *line = _lines[i];
    ...
    if (_container.verticalForm) {
        if (lines.count == 1) {
            ...
            if (top > bottom) KC_SWAP(top, bottom);
        ...
        if (lines.count == 1) {
            ...
            if (left > right) KC_SWAP(left, right);
...
- (CGRect)rectForRange:(KCTextRange *)range {
    ...
    CGRect rectUnion = ((KCTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        KCTextSelectionRect *rect = rects[i];
...
- (NSArray *)selectionRectsForRange:(KCTextRange *)range {
    ...
    if (startLineIndex > endLineIndex) KC_SWAP(startLineIndex, endLineIndex);
    KCTextLine *startLine = _lines[startLineIndex];
    KCTextLine *endLine = _lines[endLineIndex];
    ...
    KCTextSelectionRect *start = [KCTextSelectionRect new];
    if (isVertical) {
    ...
    KCTextSelectionRect *end = [KCTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        if (offsetStart > offsetEnd) KC_SWAP(offsetStart, offsetEnd);
        KCTextSelectionRect *rect = [KCTextSelectionRect new];
        if (isVertical) {
        ...
        KCTextSelectionRect *topRect = [KCTextSelectionRect new];
        ...
        KCTextSelectionRect *bottomRect = [KCTextSelectionRect new];
...
typedef NS_OPTIONS(NSUInteger, KCTextDecorationType) {
    KCTextDecorationTypeUnderline     = 1 << 0,
    KCTextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, KCTextBorderType) {
    KCTextBorderTypeBackgound = 1 << 0,
    KCTextBorderTypeNormal    = 1 << 1,
};

static CGRect KCTextMergeRectInSameLine(CGRect rect1, CGRect rect2, BOOL isVertical) {
    if (isVertical) {
...
static void KCTextGetRunsMaxMetric(CFArrayRef runs, CGFloat *xHeight, CGFloat *underlinePosition, CGFloat *lineThickness) {
...
static void KCTextDrawRun(KCTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
    BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);
    
    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(KCTextGlyphTransformAttributeName));
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        CGContextSaveGState(context); {
            ...
            if (isVertical) {
                ...
                for (KCTextRunGlyphRange *oneRange in runRanges) {
                    NSRange range = oneRange.glyphRangeInRun;
                    NSUInteger rangeMax = range.location + range.length;
                    KCTextRunGlyphDrawMode mode = oneRange.drawMode;
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        CGContextSaveGState(context); {
                            ...
                            if (mode) { // CJK glyph, need rotated
                                ...
                                if (mode == KCTextRunGlyphDrawModeVerticalRotateMove) {
                                    x += w;
                                    y += w;
                                }
...
static void KCTextSetLinePatternInContext(KCTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context){
    ...
    } else if (pattern == KCTextLineStylePatternCircleDot) {
...
static void KCTextDrawBorderRects(CGContextRef context, CGSize size, KCTextBorder *border, NSArray *rects, BOOL isVertical) {
    KCTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        KCTextSetLinePatternInContext(border.lineStyle, border.strokeWidth, 0, context);
        CGFloat inset = -border.strokeWidth * 0.5;
        if ((border.lineStyle & 0xFF) == KCTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == KCTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void KCTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, KCTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context); {
        if (isVertical) {
            ...
            w = (styleBase == KCTextLineStyleThick ? lineWidth * 2 : lineWidth);
            ...
            KCTextSetLinePatternInContext(style, lineWidth, position.y, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == KCTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == KCTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            } else if (styleBase == KCTextLineStyleDouble) {
            ...
            CGContextSetStrokeColorWithColor(context, color);
            KCTextSetLinePatternInContext(style, lineWidth, position.x, context);
            CGContextSetLineWidth(context, w);
            if (styleBase == KCTextLineStyleSingle) {
...
static void KCTextDrawText(KCTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                CTRunRef run = CFArrayGetValueAtIndex(runs, r);
                KCTextDrawRun(line, run, context, size, isVertical, lineRunRanges[r], verticalOffset);
            }
...
static void KCTextDrawBlockBorder(KCTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            KCTextBorder *border = attrs[KCTextBlockBorderAttributeName];
            if (!border) continue;
            
            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((KCTextLine *)lines[lineStartIndex - 1]).row == line.row) lineStartIndex--;
                else break;
            }
            
            CGRect unionRect = CGRectZero;
            NSUInteger lineStartRow = ((KCTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                KCTextLine *one = lines[lineContinueIndex];
                ...
                KCTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    KCTextBorder *nextBorder = [layout.text attribute:KCTextBlockBorderAttributeName atIndex:next.range.location];
            ...
            KCTextDrawBorderRects(context, size, border, @[[NSValue valueWithCGRect:unionRect]], isVertical);
...
static void KCTextDrawBorder(KCTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, KCTextBorderType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == KCTextBorderTypeNormal ? KCTextBorderAttributeName : KCTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            KCTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    KCTextBorder *iBorder = iAttrs[borderKey];
                    ...
                    if (isVertical) {
                        KC_SWAP(iRunPosition.x, iRunPosition.y);
            ...
            for (NSInteger re = 0, reMax = runRects.count; re < reMax; re++) {
                ...
                if (isVertical) {
                    if (fabs(rect.origin.x - curRect.origin.x) < 1) {
                        curRect = KCTextMergeRectInSameLine(rect, curRect, isVertical);
                    } else {
            ...
            KCTextDrawBorderRects(context, size, border, drawRects, isVertical);
...
static void KCTextDrawDecoration(KCTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, KCTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            KCTextDecoration *underline = attrs[KCTextUnderlineAttributeName];
            KCTextDecoration *strikethrough = attrs[KCTextStrikethroughAttributeName];
            
            BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
            if ((type & KCTextDecorationTypeUnderline) && underline.style > 0) {
                needDrawUnderline = YES;
            }
            if ((type & KCTextDecorationTypeStrikethrough) && strikethrough.style > 0) {
            ...
            if (KCTextIsLinebreakString(runStr)) continue; // may need more checks...
            
            CGFloat xHeight, underlinePosition, lineThickness;
            KCTextGetRunsMaxMetric(runs, &xHeight, &underlinePosition, &lineThickness);
            ...
            if (needDrawUnderline) {
                ...
                KCTextShadow *shadow = underline.shadow;
                while (shadow) {
                    if (!shadow.color) {
                    ...
                    CGContextSaveGState(context); {
                        ...
                        CGContextSaveGState(context); {
                            ...
                            KCTextDrawLineStyle(context, length, thickness, underline.style, underlineStart, color, isVertical);
                        } CGContextRestoreGState(context);
                    } CGContextRestoreGState(context);
                    shadow = shadow.subShadow;
                }
            ...
            if (needDrawStrikethrough) {
                ...
                KCTextDrawLineStyle(context, length, thickness, strikethrough.style, strikethroughStart, color, isVertical);
            }
        }
    }
    CGContextRestoreGState(context);
...
static void KCTextDrawAttachment(KCTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;
    
    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        KCTextAttachment *a = layout.attachments[i];
        ...
        rect = KCCGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = CGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
...
static void KCTextDrawShadow(KCTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context); {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                CTRunRef run = CFArrayGetValueAtIndex(runs, r);
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                KCTextShadow *shadow = attrs[KCTextShadowAttributeName];
                KCTextShadow *nsShadow = [KCTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void KCTextDrawInnerShadow(KCTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            KCTextShadow *shadow = attrs[KCTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void KCTextDrawDebug(KCTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, KCTextDebugOption *op) {
    UIGraphicsPushContext(context);
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / KCScreenScale());
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                if (isVertical) {
                    KC_SWAP(runPosition.x, runPosition.y);
...
- (void)drawInContext:(CGContextRef)context
    ...
    @autoreleasepool {
        if (self.needDrawBlockBorder && context) {
            ...
            KCTextDrawBlockBorder(self, context, size, point, cancel);
        }
        if (self.needDrawBackgroundBorder && context) {
            if (cancel && cancel()) return;
            KCTextDrawBorder(self, context, size, point, KCTextBorderTypeBackgound, cancel);
        }
        if (self.needDrawShadow && context) {
            if (cancel && cancel()) return;
            KCTextDrawShadow(self, context, size, point, cancel);
        }
        if (self.needDrawUnderline && context) {
            if (cancel && cancel()) return;
            KCTextDrawDecoration(self, context, size, point, KCTextDecorationTypeUnderline, cancel);
        }
        if (self.needDrawText && context) {
            if (cancel && cancel()) return;
            KCTextDrawText(self, context, size, point, cancel);
        }
        if (self.needDrawAttachment && (context || view || layer)) {
            if (cancel && cancel()) return;
            KCTextDrawAttachment(self, context, size, point, view, layer, cancel);
        }
        if (self.needDrawInnerShadow && context) {
            if (cancel && cancel()) return;
            KCTextDrawInnerShadow(self, context, size, point, cancel);
        }
        if (self.needDrawStrikethrough && context) {
            if (cancel && cancel()) return;
            KCTextDrawDecoration(self, context, size, point, KCTextDecorationTypeStrikethrough, cancel);
        }
        if (self.needDrawBorder && context) {
            if (cancel && cancel()) return;
            KCTextDrawBorder(self, context, size, point, KCTextBorderTypeNormal, cancel);
        }
        if (debug.needDrawDebug && context) {
            if (cancel && cancel()) return;
            KCTextDrawDebug(self, context, size, point, debug);
        }
    }
}
...
- (void)removeAttachmentFromViewAndLayer {
    KCAssertMainThread();
    for (KCTextAttachment *a in self.attachments) {
