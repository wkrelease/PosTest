...
static BOOL KCCGImageLastPixelFilled(CGImageRef image) {
    if (!image) return NO;
    size_t width = CGImageGetWidth(image);
    size_t height = CGImageGetHeight(image);
    if (width == 0 || height == 0) return NO;
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, KCCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface KCWebImageOperation ()<NSURLConnectionDelegate>
...
@property (nonatomic, strong) KCImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) KCWebImageProgressBlock progress;
@property (nonatomic, copy) KCWebImageTransformBlock transform;
@property (nonatomic, copy) KCWebImageCompletionBlock completion;

@end


@implementation KCWebImageOperation
...
+ (void)_networkThreadMain:(id)object {
    @autoreleasepool {
        [[NSThread currentThread] setName:@"com.ibireme.kckit.webimage.request"];
    ...
    dispatch_once(&onceToken, ^{
        ...
        if ([UIDevice currentDevice].systemVersion.floatValue >= 8.0) {
            for (NSUInteger i = 0; i < queueCount; i++) {
                ...
                queues[i] = dispatch_queue_create("com.ibireme.kckit.decode", attr);
            }
        } else {
            for (NSUInteger i = 0; i < queueCount; i++) {
                queues[i] = dispatch_queue_create("com.ibireme.kckit.decode", DISPATCH_QUEUE_SERIAL);
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"KCWebImageOperation init error" reason:@"KCWebImageOperation must be initialized with a request. Use the designated initializer to init." userInfo:nil];
...
- (void)dealloc {
    ...
    if ([self isExecuting]) {
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & KCWebImageOptionShowNetworkActivity)) {
                [[UIApplication sharedExtensionApplication] decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, KCWebImageFromNone, KCWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & KCWebImageOptionUseNSURLCache) &&
            !(_options & KCWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:KCImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, KCWebImageFromMemoryCache, KCWebImageStageCancelled, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & KCWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:KCImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:KCImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & KCWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, KCWebImageFromNone, KCWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[KCWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & KCWebImageOptionShowNetworkActivity)) {
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, KCWebImageFromNone, KCWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, KCWebImageFromDiskCache, KCWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & KCWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([KCWebImageOperation _imageQueue], ^{
                        [_cache setImage:image imageData:data forKey:_cacheKey withType:KCImageCacheTypeAll];
                    });
                }
            }
            _data = nil;
            NSError *error = nil;
            if (!image) {
                error = [NSError errorWithDomain:@"com.ibireme.kckit.image" code:-1 userInfo:@{ NSLocalizedDescriptionKey : @"Web image decode fail." }];
                if (_options & KCWebImageOptionIgnoreFailedURL) {
            ...
            if (_completion) _completion(image, _request.URL, KCWebImageFromRemote, KCWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & KCWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & KCWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & KCWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & KCWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[KCImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == KCImageTypeUnknown ||
            _progressiveDecoder.type == KCImageTypeWebP ||
            _progressiveDecoder.type == KCImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != KCImageTypeJPEG &&
                _progressiveDecoder.type != KCImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            KCImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, KCWebImageFromRemote, KCWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == KCImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (!KCCGImageLastPixelFilled(image.CGImage)) return;
            ...
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, KCWebImageFromRemote, KCWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & KCWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & KCWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[KCImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image imageByDecoded];
                    if ([((KCImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    KCImageDecoder *decoder = [KCImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                KCImageType imageType = KCImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case KCImageTypeJPEG:
                    case KCImageTypeGIF:
                    case KCImageTypePNG:
                    case KCImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == KCImageTypeGIF ||
                                imageType == KCImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & KCWebImageOptionShowNetworkActivity)) {
                [[UIApplication sharedExtensionApplication] decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, KCWebImageFromNone, KCWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & KCWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & KCWebImageOptionAllowBackgroundTask) && ![UIApplication isAppExtension]) {
