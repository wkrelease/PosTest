...
/**
 ...
 @return YES or NO.
 */
static inline BOOL KCTextIsLinebreakChar(unichar c) {
    switch (c) {
        case 0x000D:
        case 0x2028:
        case 0x000A:
...
/**
 ...
 @return YES or NO.
 */
static inline BOOL KCTextIsLinebreakString(NSString *str) {
    if (str.length > 2 || str.length == 0) return NO;
    if (str.length == 1) {
        unichar c = [str characterAtIndex:0];
        return KCTextIsLinebreakChar(c);
    } else {
 ...
 */
static inline NSUInteger KCTextLinebreakTailLength(NSString *str) {
    if (str.length >= 2) {
        unichar c2 = [str characterAtIndex:str.length - 1];
        if (KCTextIsLinebreakChar(c2)) {
        ...
        return KCTextIsLinebreakChar([str characterAtIndex:0]) ? 1 : 0;
    } else {
        return 0;
    }
}
 ...
 */
static inline CGFloat KCEmojiGetAscentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGFloat KCEmojiGetDescentWithFontSize(CGFloat fontSize) {
    if (fontSize < 16) {
 ...
 */
static inline CGRect KCEmojiGetGlyphBoundingRectWithFontSize(CGFloat fontSize) {
    CGRect rect;
    rect.origin.x = 0.75;
    rect.size.width = rect.size.height = KCEmojiGetAscentWithFontSize(fontSize);
    if (fontSize < 16) {
 ...
 */
NSCharacterSet *KCTextVerticalFormRotateCharacterSet();

/**
 Get the character set which should rotate and move in vertical form.
 @return The shared character set.
 */
NSCharacterSet *KCTextVerticalFormRotateAndMoveCharacterSet();
