...
KCSYNTH_DUMMY_CLASS(CALayer_KCWebImage)

static int _KCWebImageSetterKey;

@implementation CALayer (KCWebImage)
...
- (void)setImageWithURL:(NSURL *)imageURL
    ...
    dispatch_async_on_main_queue(^{
        if ((options & KCWebImageOptionSetImageWithFadeAnimation) &&
            ...
            [self removeAnimationForKey:_KCWebImageFadeAnimationKey];
        }
        ...
        dispatch_async([_KCWebImageSetter setterQueue], ^{
            ...
            KCWebImageCompletionBlock _completion = ^(UIImage *image, NSURL *url, KCWebImageFromType from, KCWebImageStage stage, NSError *error) {
                ...
                BOOL showFade = (options & KCWebImageOptionSetImageWithFadeAnimation);
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (setImage && self) {
                        if (showFade) {
                            ...
                            [self addAnimation:transition forKey:_KCWebImageFadeAnimationKey];
                        }
