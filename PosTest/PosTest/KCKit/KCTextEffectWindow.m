...
@implementation KCTextEffectWindow

+ (instancetype)sharedWindow {
    static KCTextEffectWindow *one = nil;
...
- (KCTextDirection)_keyboardDirection {
    CGRect keyboardFrame = [KCTextKeyboardManager defaultManager].keyboardFrame;
    keyboardFrame = [[KCTextKeyboardManager defaultManager] convertRect:keyboardFrame toView:self];
    if (CGRectIsNull(keyboardFrame) || CGRectIsEmpty(keyboardFrame)) return KCTextDirectionNone;
    
    if (CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return KCTextDirectionTop;
    ...
    if (CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame) &&
        ...
        return KCTextDirectionRight;
    
    if (CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame) &&
        CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMaxX(keyboardFrame) == CGRectGetWidth(self.frame))
        return KCTextDirectionBottom;
    
    if (CGRectGetMinX(keyboardFrame) == 0 &&
        CGRectGetMinY(keyboardFrame) == 0 &&
        CGRectGetMaxY(keyboardFrame) == CGRectGetHeight(self.frame))
        return KCTextDirectionLeft;
    
    return KCTextDirectionNone;
}
...
- (CGPoint)_correctedCenter:(CGPoint)center forMagnifier:(KCTextMagnifier *)mag rotation:(CGFloat)rotation {
    ...
    if (degree <= 1 || degree >= 7) { //top
        if (mag.type == KCTextMagnifierTypeCaret) {
            if (center.y < caretExt)
                center.y = caretExt;
        } else if (mag.type == KCTextMagnifierTypeRanged) {
...
- (CGFloat)_updateMagnifier:(KCTextMagnifier *)mag {
    ...
    CGAffineTransform trans = KCCGAffineTransformGetFromViews(hostView, self);
    ...
    for (UIWindow *window in windows) {
        ...
        CGContextConcatCTM(context, KCCGAffineTransformGetFromViews(window, self));
...
- (void)showMagnifier:(KCTextMagnifier *)mag {
    ...
    mag.transform = trans;
    mag.center = center;
    if (mag.type == KCTextMagnifierTypeRanged) {
        mag.alpha = 0;
    }
    NSTimeInterval time = mag.type == KCTextMagnifierTypeCaret ? 0.06 : 0.1;
...
- (void)hideMagnifier:(KCTextMagnifier *)mag {
    ...
    CGFloat rotation = [self _updateMagnifier:mag];
    CGPoint center = [self convertPoint:mag.hostPopoverCenter fromViewOrWindow:mag.hostView];
    NSTimeInterval time = mag.type == KCTextMagnifierTypeCaret ? 0.20 : 0.15;
    [UIView animateWithDuration:time delay:0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction | UIViewAnimationOptionBeginFromCurrentState animations:^{
...
- (void)_updateSelectionGrabberDot:(KCSelectionGrabberDot *)dot selection:(KCTextSelectionView *)selection{
