...
@implementation KCKVStorageItem

@end


@implementation KCKVStorage{
...
- (KCKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    KCKVStorageItem *item = [KCKVStorageItem new];
...
- (KCKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    KCKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            KCKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"KCKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:nil type:KCKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(KCKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"KCKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > KCKVStorageTypeMixed) {
        NSLog(@"KCKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
        ...
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
            NSLog(@"KCKVStorage init error:%@", error);
            return nil;
        }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"KCKVStorage init error: fail to open sqlite db.");
        }
        return nil;
    }
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == KCKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != KCKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case KCKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case KCKVStorageTypeFile:
        case KCKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (KCKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (KCKVStorageItem *)getItemForKey:(NSString *)key {
    ...
    KCKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (KCKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    KCKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
