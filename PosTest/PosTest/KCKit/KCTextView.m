...
NSString *const KCTextViewTextDidBeginEditingNotification = @"KCTextViewTextDidBeginEditing";
NSString *const KCTextViewTextDidChangeNotification = @"KCTextViewTextDidChange";
NSString *const KCTextViewTextDidEndEditingNotification = @"KCTextViewTextDidEndEditing";



typedef NS_ENUM (NSUInteger, KCTextGrabberDirection) {
    kStart = 1,
    kEnd   = 2,
};

typedef NS_ENUM(NSUInteger, KCTextMoveDirection) {
    kLeft   = 1,
    kTop    = 2,
    kRight  = 3,
    kBottom = 4,
...
@interface _KCTextViewUndoObject : NSObject
@property (nonatomic, strong) NSAttributedString *text;
@property (nonatomic, assign) NSRange selectedRange;
@end
@implementation _KCTextViewUndoObject
+ (instancetype)objectWithText:(NSAttributedString *)text range:(NSRange)range {
    _KCTextViewUndoObject *obj = [self new];
...
@interface KCTextView ()<UIScrollViewDelegate, UIAlertViewDelegate, KCTextDebugTarget, KCTextKeyboardObserver>{
    
    KCTextRange *_selectedTextRange; /// nonnull
    KCTextRange *_markedTextRange;
    
    __weak id<KCTextViewDelegate> _outerDelegate;
    ...
    KCTextContainer *_innerContainer; ///< nonnull, inner text container
    KCTextLayout *_innerLayout; ///< inner text layout, the text in this layout is longer than `_innerText` by appending '\n'
    
    KCTextContainerView *_containerView; ///< nonnull
    KCTextSelectionView *_selectionView; ///< nonnull
    KCTextMagnifier *_magnifierCaret; ///< nonnull
    KCTextMagnifier *_magnifierRanged; ///< nonnull
    ...
    KCTextHighlight *_highlight; ///< highlight attribute in `_highlightRange`
    KCTextLayout *_highlightLayout; ///< when _state.showingHighlight=YES, this layout should be displayed
    KCTextRange *_trackingRange; ///< the range in _innerLayout, may out of _innerText.
    ...
    struct {
        unsigned int trackingGrabber : 2;       ///< KCTextGrabberDirection, current tracking grabber
        ...
        unsigned int touchMoved : 3;            ///< KCTextMoveDirection, move direction after touch began
...
@synthesize selectedTextRange = _selectedTextRange;  //copy nonnull (KCTextRange*)
@synthesize markedTextRange = _markedTextRange;      //readonly     (KCTextRange*)
...
- (void)_commitUpdate {
    ...
    [[KCTransaction transactionWithTarget:self selector:@selector(_updateIfNeeded)] commit];
}
...
- (void)_updateLayout {
    ...
    [text removeAttribute:KCTextBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    [text removeAttribute:KCTextBackgroundBorderAttributeName range:NSMakeRange(_innerText.length, 1)];
    ...
    _innerLayout = [KCTextLayout layoutWithContainer:_innerContainer text:text];
...
- (void)_updateSelectionView {
    ...
    [[KCTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    if (!_innerLayout) return;
    
    NSMutableArray *allRects = [NSMutableArray new];
    BOOL containsDot = NO;
    
    KCTextRange *selectedRange = _selectedTextRange;
    ...
    [allRects enumerateObjectsUsingBlock:^(KCTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
    ...
    if (!_state.firstShowDot && containsDot) {
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.02 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [[KCTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
        });
    }
    [[KCTextEffectWindow sharedWindow] showSelectionDot:_selectionView];
...
- (void)_commitPlaceholderUpdate {
    ...
    [[KCTransaction transactionWithTarget:self selector:@selector(_updatePlaceholderIfNeeded)] commit];
}
...
- (void)_updatePlaceholder {
    ...
    if (_placeholderAttributedText.length > 0) {
        KCTextContainer *container = _innerContainer.copy;
        container.size = self.bounds.size;
        container.truncationType = KCTextTruncationTypeEnd;
        container.truncationToken = nil;
        KCTextLayout *layout = [KCTextLayout layoutWithContainer:_innerContainer text:_placeholderAttributedText];
...
- (void)_updateTextRangeByTrackingCaret {
    ...
    KCTextPosition *newPos = [_innerLayout closestPositionToPoint:trackingPoint];
    if (newPos) {
        ...
        KCTextRange *newRange = [KCTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
...
- (void)_updateTextRangeByTrackingGrabber {
    ...
    KCTextPosition *position = [_innerLayout positionForPoint:magPoint
    ...
    if (position) {
        ...
        if ((NSUInteger)position.offset > _innerText.length) {
            position = [KCTextPosition positionWithOffset:_innerText.length];
        }
        KCTextRange *newRange = [KCTextRange rangeWithStart:(isStart ? position : _selectedTextRange.start)
...
- (void)_updateTextRangeByTrackingPreSelect {
    if (!_state.trackingTouch) return;
    KCTextRange *newRange = [self _getClosestTokenRangeAtPoint:_trackingPoint];
    _trackingRange = newRange;
}
...
- (void)_showMagnifierCaret {
    ...
    if (_state.showingMagnifierRanged) {
        ...
        [[KCTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    }
    ...
    if (!_state.showingMagnifierCaret) {
        ...
        [[KCTextEffectWindow sharedWindow] showMagnifier:_magnifierCaret];
    } else {
        [[KCTextEffectWindow sharedWindow] moveMagnifier:_magnifierCaret];
    }
}
...
- (void)_showMagnifierRanged {
    ...
    if (_state.showingMagnifierCaret) {
        ...
        [[KCTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    }
    ...
    if (lineIndex < _innerLayout.lines.count) {
        KCTextLine *line = _innerLayout.lines[lineIndex];
        CGRect lineRect = [self _convertRectFromLayout:line.bounds];
        if (_verticalForm) {
            magPoint.x = KC_CLAMP(magPoint.x, CGRectGetMinX(lineRect), CGRectGetMaxX(lineRect));
        } else {
            magPoint.y = KC_CLAMP(magPoint.y, CGRectGetMinY(lineRect), CGRectGetMaxY(lineRect));
        }
        ...
        if (!_state.showingMagnifierRanged) {
            ...
            [[KCTextEffectWindow sharedWindow] showMagnifier:_magnifierRanged];
        } else {
            [[KCTextEffectWindow sharedWindow] moveMagnifier:_magnifierRanged];
        }
    }
}
...
- (void)_showMenu {
    ...
    if (_selectionView.caretVisible) {
        ...
        KCTextSelectionRect *sRect = _selectionView.selectionRects.firstObject;
        rect = sRect.rect;
        ...
        KCTextKeyboardManager *mgr = [KCTextKeyboardManager defaultManager];
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        _highlightLayout = [KCTextLayout layoutWithContainer:_innerContainer text:hiText];
...
- (void)_scrollRangeToVisible:(KCTextRange *)range {
    if (!range) return;
...
- (void)_keyboardChanged {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        if ([KCTextKeyboardManager defaultManager].keyboardVisible) {
            [self _scrollRangeToVisible:_selectedTextRange];
        } else {
...
- (void)_startLongPressTimer {
    ...
    _longPressTimer = [NSTimer timerWithTimeInterval:kLongPressMinimumDuration
                                              target:[KCWeakProxy proxyWithTarget:self]
...
- (void)_trackDidLongPress {
    ...
    if (_state.showingHighlight) {
        ...
        if (_highlight.longPressAction) {
            ...
            CGRect rect = [_innerLayout rectForRange:[KCTextRange rangeWithRange:_highlightRange]];
            rect = [self _convertRectFromLayout:rect];
        ...
        if (_state.trackingTouch) {
            if (_state.trackingGrabber) {
                ...
                if (newPos) {
                    ...
                    _trackingRange = [KCTextRange rangeWithRange:NSMakeRange(newPos.offset, 0) affinity:newPos.affinity];
                    [self _updateSelectionView];
                }
                [self _hideMenu];
...
- (void)_fixSelectionDot {
    ...
    CGPoint origin = [self convertPoint:CGPointZero toViewOrWindow:[KCTextEffectWindow sharedWindow]];
...
- (KCTextRange *)_getClosestTokenRangeAtPosition:(KCTextPosition *)position {
    position = [self _correctedTextPosition:position];
    if (!position) return nil;
    KCTextRange *range = nil;
    if (_tokenizer) {
    ...
    if (range.asRange.length == 0) {
        ...
        KCTextRange *extStart = [_innerLayout textRangeByExtendingPosition:range.start];
        KCTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:range.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            range = [KCTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    range = [self _correctedTextRange:range];
    if (range.asRange.length == 0) {
        range = [KCTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
/// Try to get the character range/position with word granularity from the tokenizer.
- (KCTextRange *)_getClosestTokenRangeAtPoint:(CGPoint)point {
    point = [self _convertPointToLayout:point];
    KCTextRange *touchRange = [_innerLayout closestTextRangeAtPoint:point];
    touchRange = [self _correctedTextRange:touchRange];
    
    if (_tokenizer && touchRange) {
        KCTextRange *encEnd = (id)[_tokenizer rangeEnclosingPosition:touchRange.end withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionBackward];
        KCTextRange *encStart = (id)[_tokenizer rangeEnclosingPosition:touchRange.start withGranularity:UITextGranularityWord inDirection:UITextStorageDirectionForward];
        if (encEnd && encStart) {
            NSArray *arr = [@[encEnd.start, encEnd.end, encStart.start, encStart.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [KCTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    }
    
    if (touchRange) {
        KCTextRange *extStart = [_innerLayout textRangeByExtendingPosition:touchRange.start];
        KCTextRange *extEnd = [_innerLayout textRangeByExtendingPosition:touchRange.end];
        if (extStart && extEnd) {
            NSArray *arr = [@[extStart.start, extStart.end, extEnd.start, extEnd.end] sortedArrayUsingSelector:@selector(compare:)];
            touchRange = [KCTextRange rangeWithStart:arr.firstObject end:arr.lastObject];
        }
    ...
    if (!touchRange) touchRange = [KCTextRange defaultRange];
    
    if (_innerText.length && touchRange.asRange.length == 0) {
        touchRange = [KCTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    }
...
- (KCTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    if (!_highlightable || !_innerLayout.containsHighlight) return nil;
    point = [self _convertPointToLayout:point];
    KCTextRange *textRange = [_innerLayout textRangeAtPoint:point];
    ...
    KCTextHighlight *highlight = [text attribute:KCTextHighlightAttributeName
                                         atIndex:startIndex
...
- (CGFloat)_getMagnifierRangedOffset {
    ...
    KCTextPosition *position = [_innerLayout closestPositionToPoint:magPoint];
    NSUInteger lineIndex = [_innerLayout lineIndexForPosition:position];
    if (lineIndex < _innerLayout.lines.count) {
        KCTextLine *line = _innerLayout.lines[lineIndex];
        if (_verticalForm) {
            magPoint.x = KC_CLAMP(magPoint.x, line.left, line.right);
            return magPoint.x - line.position.x + kMagnifierRangedPopoverOffset;
        } else {
            magPoint.y = KC_CLAMP(magPoint.y, line.top, line.bottom);
...
/// Return a KCTextMoveDirection from `_touchBeganPoint` to `_trackingPoint`.
...
- (BOOL)_isTextPositionValid:(KCTextPosition *)position {
    if (!position) return NO;
    if (position.offset < 0) return NO;
    if (position.offset > _innerText.length) return NO;
    if (position.offset == 0 && position.affinity == KCTextAffinityBackward) return NO;
    if (position.offset == _innerText.length && position.affinity == KCTextAffinityBackward) return NO;
    return YES;
}

/// Whether the range is valid (not out of bounds).
- (BOOL)_isTextRangeValid:(KCTextRange *)range {
...
- (KCTextPosition *)_correctedTextPosition:(KCTextPosition *)position {
    if (!position) return nil;
    if ([self _isTextPositionValid:position]) return position;
    if (position.offset < 0) {
        return [KCTextPosition positionWithOffset:0];
    }
    if (position.offset > _innerText.length) {
        return [KCTextPosition positionWithOffset:_innerText.length];
    }
    if (position.offset == 0 && position.affinity == KCTextAffinityBackward) {
        return [KCTextPosition positionWithOffset:position.offset];
    }
    if (position.offset == _innerText.length && position.affinity == KCTextAffinityBackward) {
        return [KCTextPosition positionWithOffset:position.offset];
    }
    return position;
}
...
- (KCTextRange *)_correctedTextRange:(KCTextRange *)range {
    if (!range) return nil;
    if ([self _isTextRangeValid:range]) return range;
    KCTextPosition *start = [self _correctedTextPosition:range.start];
    KCTextPosition *end = [self _correctedTextPosition:range.end];
    return [KCTextRange rangeWithStart:start end:end];
}
...
- (CGPoint)_convertPointToLayout:(CGPoint)point {
    ...
    if (_innerLayout.container.isVerticalForm) {
        ...
        if (boundingSize.width < self.bounds.size.width) {
            if (_textVerticalAlignment == KCTextVerticalAlignmentCenter) {
                point.x += (self.bounds.size.width - boundingSize.width) * 0.5;
            } else if (_textVerticalAlignment == KCTextVerticalAlignmentBottom) {
...
- (void)_replaceRange:(KCTextRange *)range withText:(NSString *)text notifyToDelegate:(BOOL)notify{
    if (NSEqualRanges(range.asRange, _selectedTextRange.asRange)) {
        ...
        _selectedTextRange = [KCTextRange rangeWithRange:newRange];
        ...
        if (range.asRange.length != text.length) {
            ...
            if (unionRange.length == 0) {
                ...
                _selectedTextRange = [KCTextRange rangeWithRange:NSMakeRange(range.start.offset + text.length, 0)];
...
- (void)_updateAttributesHolder {
    if (_innerText.length > 0) {
        ...
        [_typingAttributesHolder removeAttribute:KCTextBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
        [_typingAttributesHolder removeAttribute:KCTextBackgroundBorderAttributeName range:NSMakeRange(0, _typingAttributesHolder.length)];
    }
}
...
- (BOOL)_parseText {
    if (self.textParser) {
        KCTextRange *oldTextRange = _selectedTextRange;
        NSRange newRange = _selectedTextRange.asRange;
        ...
        KCTextRange *newTextRange = [KCTextRange rangeWithRange:newRange];
        ...
        switch (result.resultType) {
            ...
            case NSTextCheckingTypePhoneNumber: {
                ...
                if (_highlightTextAttributes.count) {
                    KCTextHighlight *highlight = [KCTextHighlight highlightWithAttributes:_highlightTextAttributes];
...
- (void)_resetUndoAndRedoStack {
    ...
    _KCTextViewUndoObject *object = [_KCTextViewUndoObject objectWithText:_innerText.copy range:_selectedTextRange.asRange];
...
- (void)_saveToUndoStack {
    ...
    _KCTextViewUndoObject *lastObject = _undoStack.lastObject;
...
- (void)_saveToRedoStack {
    if (!_allowsUndoAndRedo) return;
    _KCTextViewUndoObject *lastObject = _redoStack.lastObject;
    if ([lastObject.text isEqualToAttributedString:self.attributedText]) return;
...
- (BOOL)_canUndo {
    ...
    _KCTextViewUndoObject *object = _undoStack.lastObject;
...
- (void)_redo {
    ...
    _KCTextViewUndoObject *object = _redoStack.lastObject;
...
- (void)_setTextParser:(id<KCTextParser>)textParser {
...
- (void)_setLinePositionModifier:(id<KCTextLinePositionModifier>)linePositionModifier {
...
- (void)_initTextView {
    ...
    // UITextInput
    _selectedTextRange = [KCTextRange defaultRange];
    ...
    _innerContainer = [KCTextContainer new];
    ...
    KCTextHighlight *highlight = [KCTextHighlight new];
    KCTextBorder * border = [KCTextBorder new];
    ...
    _containerView = [KCTextContainerView new];
    _containerView.userInteractionEnabled = NO;
    _containerView.hostView = self;
    
    _selectionView = [KCTextSelectionView new];
    _selectionView.userInteractionEnabled = NO;
    _selectionView.hostView = self;
    _selectionView.color = [self _defaultTintColor];
    
    _magnifierCaret = [KCTextMagnifier magnifierWithType:KCTextMagnifierTypeCaret];
    _magnifierCaret.hostView = _containerView;
    _magnifierRanged = [KCTextMagnifier magnifierWithType:KCTextMagnifierTypeRanged];
    ...
    self.debugOption = [KCTextDebugOption sharedDebugOption];
    [KCTextDebugOption addDebugTarget:self];
    ...
    [[KCTextKeyboardManager defaultManager] addObserver:self];
...
- (void)dealloc {
    ...
    [[KCTextKeyboardManager defaultManager] removeObserver:self];
    
    [[KCTextEffectWindow sharedWindow] hideSelectionDot:_selectionView];
    [[KCTextEffectWindow sharedWindow] hideMagnifier:_magnifierCaret];
    [[KCTextEffectWindow sharedWindow] hideMagnifier:_magnifierRanged];
    
    [KCTextDebugOption removeDebugTarget:self];
...
- (void)scrollRangeToVisible:(NSRange)range {
    KCTextRange *textRange = [KCTextRange rangeWithRange:range];
    textRange = [self _correctedTextRange:textRange];
...
- (void)setText:(NSString *)text {
    ...
    [self replaceRange:[KCTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:text];
}
...
- (void)setTextParser:(id<KCTextParser>)textParser {
    if (_textParser == textParser || [_textParser isEqual:textParser]) return;
    [self _setTextParser:textParser];
    if (textParser && _text.length) {
        [self replaceRange:[KCTextRange rangeWithRange:NSMakeRange(0, _text.length)] withText:_text];
    }
    [self _resetUndoAndRedoStack];
    [self _commitUpdate];
}
...
- (void)setAttributedText:(NSAttributedString *)attributedText {
    ...
    if (text.length == 0) {
        [self replaceRange:[KCTextRange rangeWithRange:NSMakeRange(0, _innerText.length)] withText:@""];
        return;
    }
    ...
    _innerText = text;
    [self _parseText];
    _selectedTextRange = [KCTextRange rangeWithRange:NSMakeRange(0, _innerText.length)];
    [_inputDelegate textDidChange:self];
    [_inputDelegate selectionDidChange:self];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:KCTextViewTextDidChangeNotification object:self];
}

- (void)setTextVerticalAlignment:(KCTextVerticalAlignment)textVerticalAlignment {
...
- (void)setSelectedRange:(NSRange)selectedRange {
    ...
    KCTextRange *range = [KCTextRange rangeWithRange:selectedRange];
    range = [self _correctedTextRange:range];
    [self _endTouchTracking];
...
- (void)setDebugOption:(KCTextDebugOption *)debugOption {
    _containerView.debugOption = debugOption;
}

- (KCTextDebugOption *)debugOption {
    return _containerView.debugOption;
}

- (KCTextLayout *)textLayout {
...
- (CGSize)sizeThatFits:(CGSize)size {
    KCTextLayout *textLayout = self.textLayout;
    ...
    KCTextContainer *newContainer = _innerContainer.mutableCopy;
    newContainer.size = size;
    KCTextLayout *newLayout = [KCTextLayout layoutWithContainer:newContainer text:textLayout.text];
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            ...
            KCTextHighlight *highlight = [self _getHighlightAtPoint:_trackingPoint range:NULL];
            ...
            if (_state.trackingCaret) {
                ...
                if (!_state.touchMoved) {
                    if (_state.selectedWithoutEdit) {
                        ...
                        if (self.isFirstResponder) {
                            KCTextRange *_oldRange = _trackingRange;
                            [self _updateTextRangeByTrackingCaret];
...
- (BOOL)becomeFirstResponder {
    ...
    if (!isFirstResponder && become) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:KCTextViewTextDidBeginEditingNotification object:self];
    }
    return become;
}
...
- (BOOL)resignFirstResponder {
    ...
    if (resign) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:KCTextViewTextDidEndEditingNotification object:self];
    }
    return resign;
}
...
- (void)paste:(id)sender {
    ...
    if (!atr && _allowsPasteImage) {
        UIImage *img = nil;
        if (p.GIFData) {
            img = [KCImage imageWithData:p.GIFData scale:kScreenScale];
        }
        if (!img && p.PNGData) {
            img = [KCImage imageWithData:p.PNGData scale:kScreenScale];
        }
        if (!img && p.WEBPData) {
            img = [KCImage imageWithData:p.WEBPData scale:kScreenScale];
        }
        if (!img) {
            img = p.image;
        }
        ...
        if (img && img.size.width > 1 && img.size.height > 1) {
            id content = img;
            if ([img conformsToProtocol:@protocol(KCAnimatedImage)]) {
                id<KCAnimatedImage> ani = (id)img;
                if (ani.animatedImageFrameCount > 1) {
                    KCAnimatedImageView *aniView = [[KCAnimatedImageView alloc] initWithImage:img];
                    if (aniView) {
                        content = aniView;
                    }
                }
    ...
    if (atr) {
        ...
        KCTextPosition *pos = [self _correctedTextPosition:[KCTextPosition positionWithOffset:endPosition]];
        KCTextRange *range = [_innerLayout textRangeByExtendingPosition:pos];
        range = [self _correctedTextRange:range];
        if (range) {
...
- (void)select:(id)sender {
    ...
    KCTextRange *newRange = [self _getClosestTokenRangeAtPosition:_selectedTextRange.start];
...
- (id<KCTextViewDelegate>)delegate {
    return _outerDelegate;
}

- (void)setDelegate:(id<KCTextViewDelegate>)delegate {
...
#pragma mark - @protocol KCTextKeyboardObserver
...
- (void)deleteBackward {
    ...
    if (!_state.deleteConfirm && range.length == 0 && range.location > 0) {
        ...
        KCTextBinding *binding = [_innerText attribute:KCTextBindingAttributeName atIndex:range.location - 1 longestEffectiveRange:&effectiveRange inRange:NSMakeRange(0, _innerText.length)];
        if (binding && binding.deleteConfirm) {
            _state.deleteConfirm = YES;
            [_inputDelegate selectionWillChange:self];
            _selectedTextRange = [KCTextRange rangeWithRange:effectiveRange];
            _selectedTextRange = [self _correctedTextRange:_selectedTextRange];
            [_inputDelegate selectionDidChange:self];
    ...
    _state.deleteConfirm = NO;
    if (range.length == 0) {
        KCTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:_selectedTextRange.end inDirection:UITextLayoutDirectionLeft offset:1];
    ...
    [self replaceRange:[KCTextRange rangeWithRange:range] withText:@""];
}
...
- (void)setSelectedTextRange:(KCTextRange *)selectedTextRange {
...
- (void)setMarkedText:(NSString *)markedText selectedRange:(NSRange)selectedRange {
    ...
    if (_markedTextRange == nil) {
        _markedTextRange = [KCTextRange rangeWithRange:NSMakeRange(_selectedTextRange.end.offset, markedText.length)];
        [_innerText replaceCharactersInRange:NSMakeRange(_selectedTextRange.end.offset, 0) withString:markedText];
        _selectedTextRange = [KCTextRange rangeWithRange:NSMakeRange(_selectedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    } else {
        ...
        _markedTextRange = [KCTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset, markedText.length)];
        _selectedTextRange = [KCTextRange rangeWithRange:NSMakeRange(_markedTextRange.start.offset + selectedRange.location, selectedRange.length)];
    }
...
- (void)replaceRange:(KCTextRange *)range withText:(NSString *)text {
    if (!range) return;
    if (!text) text = @"";
...
- (void)setBaseWritingDirection:(UITextWritingDirection)writingDirection forRange:(KCTextRange *)range {
    if (!range) return;
    range = [self _correctedTextRange:range];
...
- (NSString *)textInRange:(KCTextRange *)range {
    range = [self _correctedTextRange:range];
    if (!range) return @"";
    return [_innerText.string substringWithRange:range.asRange];
}

- (UITextWritingDirection)baseWritingDirectionForPosition:(KCTextPosition *)position inDirection:(UITextStorageDirection)direction {
    [self _updateIfNeeded];
    position = [self _correctedTextPosition:position];
...
- (KCTextPosition *)beginningOfDocument {
    return [KCTextPosition positionWithOffset:0];
}

- (KCTextPosition *)endOfDocument {
    return [KCTextPosition positionWithOffset:_innerText.length];
}

- (KCTextPosition *)positionFromPosition:(KCTextPosition *)position offset:(NSInteger)offset {
    ...
    if (newLocation != 0 && newLocation != _innerText.length) {
        // fix emoji
        [self _updateIfNeeded];
        KCTextRange *extendRange = [_innerLayout textRangeByExtendingPosition:[KCTextPosition positionWithOffset:newLocation]];
    ...
    KCTextPosition *p = [KCTextPosition positionWithOffset:newLocation];
    return [self _correctedTextPosition:p];
}

- (KCTextPosition *)positionFromPosition:(KCTextPosition *)position inDirection:(UITextLayoutDirection)direction offset:(NSInteger)offset {
    [self _updateIfNeeded];
    KCTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:offset];
    ...
    KCTextPosition *newPosition = forward ? range.end : range.start;
    if (newPosition.offset > _innerText.length) {
        newPosition = [KCTextPosition positionWithOffset:_innerText.length affinity:KCTextAffinityBackward];
    }
    
    return [self _correctedTextPosition:newPosition];
}

- (KCTextRange *)textRangeFromPosition:(KCTextPosition *)fromPosition toPosition:(KCTextPosition *)toPosition {
    return [KCTextRange rangeWithStart:fromPosition end:toPosition];
}

- (NSComparisonResult)comparePosition:(KCTextPosition *)position toPosition:(KCTextPosition *)other {
    return [position compare:other];
}

- (NSInteger)offsetFromPosition:(KCTextPosition *)from toPosition:(KCTextPosition *)toPosition {
    return toPosition.offset - from.offset;
}

- (KCTextPosition *)positionWithinRange:(KCTextRange *)range farthestInDirection:(UITextLayoutDirection)direction {
    NSRange nsRange = range.asRange;
    if (direction == UITextLayoutDirectionLeft | direction == UITextLayoutDirectionUp) {
        return [KCTextPosition positionWithOffset:nsRange.location];
    } else {
        return [KCTextPosition positionWithOffset:nsRange.location + nsRange.length affinity:KCTextAffinityBackward];
    }
}

- (KCTextRange *)characterRangeByExtendingPosition:(KCTextPosition *)position inDirection:(UITextLayoutDirection)direction {
    [self _updateIfNeeded];
    KCTextRange *range = [_innerLayout textRangeByExtendingPosition:position inDirection:direction offset:1];
    return [self _correctedTextRange:range];
}
...
- (KCTextPosition *)closestPositionToPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    KCTextPosition *position = [_innerLayout closestPositionToPoint:point];
    return [self _correctedTextPosition:position];
}

- (KCTextPosition *)closestPositionToPoint:(CGPoint)point withinRange:(KCTextRange *)range {
    KCTextPosition *pos = (id)[self closestPositionToPoint:point];
    if (!pos) return nil;
...
- (KCTextRange *)characterRangeAtPoint:(CGPoint)point {
    [self _updateIfNeeded];
    point = [self _convertPointToLayout:point];
    KCTextRange *r = [_innerLayout closestTextRangeAtPoint:point];
...
- (NSArray *)selectionRectsForRange:(KCTextRange *)range {
    ...
    [rects enumerateObjectsUsingBlock:^(KCTextSelectionRect *rect, NSUInteger idx, BOOL *stop) {
        rect.rect = [self _convertRectFromLayout:rect.rect];
    }];
    return rects;
}
...
- (UITextStorageDirection)selectionAffinity {
    if (_selectedTextRange.end.affinity == KCTextAffinityForward) {
...
- (void)setSelectionAffinity:(UITextStorageDirection)selectionAffinity {
    _selectedTextRange = [KCTextRange rangeWithRange:_selectedTextRange.asRange affinity:selectionAffinity == UITextStorageDirectionForward ? KCTextAffinityForward : KCTextAffinityBackward];
    [self _updateSelectionView];
}

- (NSDictionary *)textStylingAtPosition:(KCTextPosition *)position inDirection:(UITextStorageDirection)direction {
    if (!position) return nil;
...
- (KCTextPosition *)positionWithinRange:(KCTextRange *)range atCharacterOffset:(NSInteger)offset {
    if (!range) return nil;
    ...
    else return [KCTextPosition positionWithOffset:offset];
}

- (NSInteger)characterOffsetOfPosition:(KCTextPosition *)position withinRange:(KCTextRange *)range {
